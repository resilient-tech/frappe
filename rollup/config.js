	...
	return {
		inputOptions: {
			...
			onwarn({ code, message, loc, frame }) {
				// skip warnings
				if (
					["EVAL", "SOURCEMAP_BROKEN", "NAMESPACE_CONFLICT"].includes(
						code
					)
				)
					return;
				...
				if ("UNRESOLVED_IMPORT" === code) {
					...
					log(
						`Cannot find some dependencies. You may have to run "${command}" to install them.`
					);
					log();
					return;
				}
	...
	return Object.keys(build_json)
		.map((output_file) => {
			if (output_file === file) {
				...
				const input_files = build_json[output_file].map(
					(input_file) => {
...
function get_options_for(app) {
	...
	return Object.keys(build_json)
		.map((output_file) => {
			...
			const input_files = files.map((input_file) => {
				let prefix = get_app_path(app);
				if (input_file.startsWith("node_modules/")) {
					prefix = path.resolve(get_app_path(app), "..");
				}
			...
			return Object.assign(get_rollup_options(output_file, input_files), {
				output_file,
			});
		})
		.filter(Boolean);
