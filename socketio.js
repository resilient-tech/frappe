...
io.on("connection", function (socket) {
	...
	socket.on("upload-accept-slice", (data) => {
		try {
			...
			if (
				socket.files[data.name].slice * 24576 >=
				socket.files[data.name].size
			) {
				// do something with the data
				var fileBuffer = Buffer.concat(socket.files[data.name].data);

				const file_url = path.join(
					socket.files[data.name].is_private ? "private" : "public",
					"files",
					data.name
				);
...
function get_site_name(socket) {
	if (socket.request.headers["x-frappe-site-name"]) {
		...
		["localhost", "127.0.0.1"].indexOf(socket.request.headers.host) !==
			-1 &&
		conf.default_site
	) {
...
function send_users(args, action) {
	...
	const open_doc_room = get_open_doc_room(
		args.socket,
		args.doctype,
		args.docname
	);
