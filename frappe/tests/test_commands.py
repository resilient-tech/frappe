...
def exists_in_backup(doctypes, file):
    ...
    return all(
        [predicate.format(doctype).lower() in content.lower() for doctype in doctypes]
    )
...
class TestCommands(BaseTestCommands):
    ...
    def test_restore(self):
        ...
        self.execute(
            "bench --site {another_site} backup --ignore-backup-conf", site_data
        )
        self.execute(
            "bench --site {another_site} execute"
            " frappe.utils.backups.fetch_latest_backups",
            site_data,
        )
