...
class TestDB(unittest.TestCase):
    def test_get_value(self):
        self.assertEqual(
            frappe.db.get_value("User", {"name": ["=", "Administrator"]}),
            "Administrator",
        )
        self.assertEqual(
            frappe.db.get_value("User", {"name": ["like", "Admin%"]}), "Administrator"
        )
        self.assertNotEquals(
            frappe.db.get_value("User", {"name": ["!=", "Guest"]}), "Guest"
        )
        self.assertEqual(
            frappe.db.get_value("User", {"name": ["<", "B"]}), "Administrator"
        )
        self.assertEqual(
            frappe.db.get_value("User", {"name": ["<=", "Administrator"]}),
            "Administrator",
    ...
    def test_set_value(self):
        todo1 = frappe.get_doc(
            dict(doctype="ToDo", description="test_set_value 1")
        ).insert()
        todo2 = frappe.get_doc(
            dict(doctype="ToDo", description="test_set_value 2")
        ).insert()

        frappe.db.set_value(
            "ToDo", todo1.name, "description", "test_set_value change 1"
        )
        self.assertEqual(
            frappe.db.get_value("ToDo", todo1.name, "description"),
        ...
        frappe.db.set_value(
            "ToDo",
            dict(description=("like", "%test_set_value%")),
            "description",
            "change 2",
        )

        self.assertEqual(
            frappe.db.get_value("ToDo", todo1.name, "description"), "change 2"
        )
        self.assertEqual(
            frappe.db.get_value("ToDo", todo2.name, "description"), "change 2"
        )
    ...
    def test_db_keywords_as_fields(self):
        ...
        for _ in range(10):
            ...
            doc = frappe.get_doc(
                {"doctype": test_doctype, "description": random_string(20), **docfields}
            )
            doc.insert()
        ...
        self.assertEqual(
            list(
                frappe.get_all(
                    "ToDo", fields=["`{0}` as total".format(random_field)], limit=1
                )[0]
            )[0],
            "total",
        )
        ...
        self.assertEqual(
            list(
                frappe.get_all(
                    "ToDo", fields=["count(`{0}`)".format(random_field)], limit=1
                )[0]
            )[0],
            "count"
            if frappe.conf.db_type == "postgres"
            else "count(`{0}`)".format(random_field),
        )

        # Testing update
        frappe.db.set_value(test_doctype, random_doc, random_field, random_value)
        self.assertEqual(
            frappe.db.get_value(test_doctype, random_doc, random_field), random_value
        )
