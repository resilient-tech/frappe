...
class RazorpaySettings(Document):
    ...
    def validate_transaction_currency(self, currency):
        if currency not in self.supported_currencies:
            frappe.throw(
                _(
                    "Please select another payment method. Razorpay does not support"
    ...
    def setup_subscription(self, settings, **kwargs):
        ...
        subscription_details = {
            ...
            "customer_notify": kwargs.get("subscription_details").get(
                "customer_notify"
            ),
        }
    ...
    def get_payment_url(self, **kwargs):
        ...
        return get_url(
            "./integrations/razorpay_checkout?token={0}".format(
        ...
        try:
            ...
            return {
                "redirect_to": frappe.redirect_to_message(
                    _("Server Error"),
                    _(
                        "Seems issue with server's razorpay config. Don't worry, in"
                        " case of failure amount will get refunded to your account."
                    ),
                ),
                "status": 401,
            }
    ...
    def verify_signature(self, body, signature, key):
        ...
        if not result:
            frappe.throw(
                _("Razorpay Signature Verification Failed"), exc=frappe.PermissionError
            )
...
def capture_payment(is_sandbox=False, sanbox_response=None):
    ...
    for doc in frappe.get_all(
        ...
        try:
            ...
            if resp.get("status") == "captured":
                frappe.db.set_value(
                    "Integration Request", doc.name, "status", "Completed"
                )
