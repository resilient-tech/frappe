...
class PaytmSettings(Document):
    ...
    def validate_transaction_currency(self, currency):
        if currency not in self.supported_currencies:
            frappe.throw(
                _(
                    "Please select another payment method. Paytm does not support"
...
def verify_transaction(**paytm_params):
    ...
    if is_valid_checksum and paytm_params.get("RESPCODE") == "01":
        ...
        frappe.respond_as_web_page(
            "Payment Failed",
            "Transaction failed to complete. In case of any deductions, deducted amount"
            " will get refunded to your account.",
            http_status_code=401,
            indicator_color="red",
        )
        frappe.log_error(
            "Order unsuccessful. Failed Response:" + cstr(paytm_params),
