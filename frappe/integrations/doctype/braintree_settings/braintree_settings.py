...
class BraintreeSettings(Document):
    ...
    def on_update(self):
        ...
        call_hook_method(
            "payment_gateway_enabled", gateway="Braintree-" + self.gateway_name
        )
    ...
    def configure_braintree(self):
        ...
        braintree.Configuration.configure(
            ...
            private_key=self.get_password(
                fieldname="private_key", raise_exception=False
            ),
        )
    ...
    def validate_transaction_currency(self, currency):
        if currency not in self.supported_currencies:
            frappe.throw(
                _(
                    "Please select another payment method. Stripe does not support"
    ...
    def get_payment_url(self, **kwargs):
        return get_url(
            "./integrations/braintree_checkout?{0}".format(urlencode(kwargs))
        )
        ...
        try:
            ...
            return {
                "redirect_to": frappe.redirect_to_message(
                    _("Server Error"),
                    _(
                        "There seems to be an issue with the server's braintree"
                        " configuration. Don't worry, in case of failure, the amount"
    ...
    def create_charge_on_braintree(self):
        ...
        if result.is_success:
            ...
            self.integration_request.db_set(
                "error", error_log.error, update_modified=False
            )
        else:
            self.integration_request.db_set("status", "Failed", update_modified=False)
            ...
            if self.data.reference_doctype and self.data.reference_docname:
                ...
                try:
                    ...
                    if braintree_success_page:
                        custom_redirect_to = frappe.get_attr(
                            braintree_success_page[-1]
                        )(self.data)
                except Exception:
                    frappe.log_error(frappe.get_traceback())
