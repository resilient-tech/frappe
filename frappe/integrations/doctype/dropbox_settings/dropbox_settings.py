...
def take_backup_to_dropbox(retry_count=0, upload_db_backup=True):
    ...
    try:
        if cint(frappe.db.get_value("Dropbox Settings", None, "enabled")):
            ...
            if cint(
                frappe.db.get_value(
                    "Dropbox Settings", None, "send_email_for_successful_backup"
                )
            ):
    ...
    if upload_db_backup:
        ...
        if dropbox_settings["no_of_backups"]:
            delete_older_backups(
                dropbox_client, "/database", dropbox_settings["no_of_backups"]
            )
...
def update_file_dropbox_status(file_name):
    frappe.db.set_value(
        "File", file_name, "uploaded_to_dropbox", 1, update_modified=False
    )
...
def get_dropbox_settings(redirect_uri=False):
    ...
    app_details = {
        ...
        "app_secret": settings.get_password(
            fieldname="app_secret_key", raise_exception=False
        )
        if settings.app_secret_key
        else frappe.conf.dropbox_secret_key,
        "access_token": settings.get_password(
            "dropbox_access_token", raise_exception=False
        )
        if settings.dropbox_access_token
        else "",
        "access_key": settings.get_password(
            "dropbox_access_key", raise_exception=False
        ),
        "access_secret": settings.get_password(
            "dropbox_access_secret", raise_exception=False
        ),
        "file_backup": settings.file_backup,
        "no_of_backups": settings.no_of_backups
        if settings.limit_no_of_backups
        else None,
    }
...
def get_redirect_url():
    if not frappe.conf.dropbox_broker_site:
        frappe.conf.dropbox_broker_site = "https://dropbox.erpnext.com"
    url = (
        "{0}/api/method/dropbox_erpnext_broker.www.setup_dropbox.get_authotize_url"
        .format(frappe.conf.dropbox_broker_site)
    )
    ...
    try:
        ...
        frappe.throw(
            _(
                "Something went wrong while generating dropbox access token. Please"
...
def generate_oauth2_access_token_from_oauth1_token(dropbox_settings=None):
    if not dropbox_settings.get("access_key") or not dropbox_settings.get(
        "access_secret"
    ):
        return {}
