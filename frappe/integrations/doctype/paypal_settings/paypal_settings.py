...
class PayPalSettings(Document):
    ...
    def validate_transaction_currency(self, currency):
        if currency not in self.supported_currencies:
            frappe.throw(
                _(
                    "Please select another payment method. PayPal does not support"
    ...
    def get_payment_url(self, **kwargs):
        ...
        if self.paypal_sandbox or self.use_sandbox:
            ...
            return_url = (
                "https://www.paypal.com/cgi-bin/webscr?cmd=_express-checkout&token={0}"
            )
    ...
    def execute_set_express_checkout(self, **kwargs):
        ...
        params.update(
            {
                ...
                "returnUrl": get_url(
                    "{0}.get_express_checkout_details".format(api_path)
                ),
        ...
        if response.get("ACK")[0] != "Success":
            frappe.throw(
                _(
                    "Looks like something is wrong with this site's Paypal"
                    " configuration."
                )
            )
    ...
    try:
        ...
        if response.get("ACK")[0] != "Success":
            frappe.respond_as_web_page(
                ...
                _(
                    "Looks like something went wrong during the transaction. Since we"
                    " haven't confirmed the payment, Paypal will automatically refund"
                    " you this amount. If it doesn't, please send us an email and"
                    " mention the Correlation ID: {0}."
...
def confirm_payment(token):
    try:
        ...
        if response.get("ACK")[0] == "Success":
            ...
            redirect_url = (
                "/integrations/payment-success?doctype={0}&docname={1}".format(
                    data.get("reference_doctype"), data.get("reference_docname")
                )
            )
        else:
...
def create_recurring_profile(token, payerid):
    try:
        ...
        starts_at = (
            get_datetime(subscription_details.get("start_date"))
...
def get_redirect_uri(doc, token, payerid):
    ...
    if data.get("subscription_details") or data.get("subscription_id"):
        return get_url(
            "{0}.create_recurring_profile?token={1}&payerid={2}".format(
                api_path, token, payerid
            )
        )
    else:
...
def manage_recurring_payment_profile_status(profile_id, action, args, url):
    ...
    if (
        response.get("ACK")[0] != "Success"
        and response.get("L_ERRORCODE0", [])[0] != "11556"
    ):
...
def handle_subscription_notification(doctype, docname):
    call_hook_method(
        "handle_subscription_notification", doctype=doctype, docname=docname
    )
