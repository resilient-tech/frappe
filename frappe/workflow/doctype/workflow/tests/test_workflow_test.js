...
QUnit.test("Test Workflow", function (assert) {
	...
	frappe.run_serially([
		...
		() => {
			let review = $(
				`.dropdown-menu li:contains("Review"):visible`
			).size();
			let approve = $(
				`.dropdown-menu li:contains("Approve"):visible`
			).size();
		...
		() => {
			assert.equal(
				$(".msgprint").text(),
				"Did not saveInsufficient Permission for User",
		...
		() => {
			let reject = $(
				`.dropdown-menu li:contains("Reject"):visible`
			).size();
