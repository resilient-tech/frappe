...
def print_has_permission_check_logs(func):
    def inner(*args, **kwargs):
        ...
        self_perm_check = (
            ...
            else kwargs.get("user") == frappe.session.user
        )
...
def has_permission(
    ...
    if doc:
        ...
        if not perm:
            push_perm_check_log(
                _(
                    "User {0} does not have doctype access via role permission for"
                    " document {1}"
        ...
        if ptype in ("read", "write", "share", "email", "print"):
            ...
            if doc:
                ...
                if doc_name in shared:
                    if ptype in ("read", "write", "share") or meta.permissions[0].get(
                        ptype
                    ):
                        return True
...
def get_doctypes_with_read():
    return list(
        set(
            [
                p.parent if type(p.parent) == str else p.parent.encode("UTF8")
...
def get_roles(user=None, with_standard=True):
    ...
    def get():
        if user == "Administrator":
            return [
                r[0] for r in frappe.db.sql("select name from `tabRole`")
...
def can_import(doctype, raise_exception=False):
    if not (
        "System Manager" in frappe.get_roles() or has_permission(doctype, "import")
    ):
        if raise_exception:
...
def can_export(doctype, raise_exception=False):
    if "System Manager" in frappe.get_roles():
        ...
        has_access = role_permissions.get("export") or role_permissions.get(
            "if_owner"
        ).get("export")
...
def update_permission_property(
    doctype, role, permlevel, ptype, value=None, validate=True
):
...
def add_permission(doctype, role, permlevel=0):
    ...
    if frappe.db.get_value(
        "Custom DocPerm",
        dict(parent=doctype, role=role, permlevel=permlevel, if_owner=0),
    ):
        return
...
def get_allowed_docs_for_doctype(user_permissions, doctype):
    ...
    return filter_allowed_docs_for_doctype(
        user_permissions, doctype, with_default_doc=False
    )
