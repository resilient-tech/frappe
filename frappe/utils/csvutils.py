...
def read_csv_content_from_attached_file(doc):
    ...
    try:
        ...
        return read_csv_content(
            fcontent, frappe.form_dict.get("ignore_encoding_errors")
        )
    except Exception:
        frappe.throw(
            _("Unable to open attached file. Did you export it as CSV?"),
            title=_("Invalid CSV Format"),
        )
...
def read_csv_content(fcontent, ignore_encoding=False):
    ...
    if not isinstance(fcontent, text_type):
        ...
        if not decoded:
            frappe.msgprint(
                _("Unknown file encoding. Tried utf-8, windows-1250, windows-1252."),
