...
def get_versions():
    ...
    for app in frappe.get_installed_apps(sort=True):
        ...
        if versions[app]["branch"] != "master":
            branch_version = app_hooks.get(
                "{0}_version".format(versions[app]["branch"])
            )
            if branch_version:
    ...
    if cache.sismember("update-user-set", user):
        for update_type in updates:
            ...
            for app in updates[update_type]:
                ...
                release_links += (
                    "<b>{title}</b>: <a"
                    " href='https://github.com/{org_name}/{app_name}/releases/tag/v{available_version}'>v{available_version}</a><br>"
                    .format(
            ...
            if release_links:
                ...
                update_message += (
                    "<div class='new-version-log'>{0}<div"
                    " class='new-version-links'>{1}</div></div>".format(
