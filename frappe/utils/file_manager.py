...
def save_file(fname, content, dt, dn, folder=None, decode=False, is_private=0, df=None):
    ...
    if not file_data:
        ...
        write_file_method = get_hook_method(
            "write_file", fallback=save_file_on_filesystem
        )
...
def write_file(content, fname, is_private=0):
    ...
    with open(
        os.path.join(file_path.encode("utf-8"), fname.encode("utf-8")), "wb+"
    ) as f:
        f.write(content)
...
def remove_file(
    fid, attached_to_doctype=None, attached_to_name=None, from_delete=False
):
...
def extract_images_from_html(doc, content):
    ...
    def _save_file(match):
        ...
        # TODO fix this
        file_url = save_file(filename, content, doctype, name, decode=True).get(
            "file_url"
        )
