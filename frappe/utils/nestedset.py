...
def update_add_node(doc, parent, parent_field):
    ...
    if parent:
        left, right = frappe.db.sql(
            "select lft, rgt from `tab{0}` where name=%s".format(doctype), parent
        )[0]
    ...
    frappe.db.sql(
        "update `tab{0}` set rgt = rgt+2, modified=%s where rgt >= %s".format(doctype),
        (n, right),
    )
    frappe.db.sql(
        "update `tab{0}` set lft = lft+2, modified=%s where lft >= %s".format(doctype),
        (n, right),
    )
    ...
    frappe.db.sql(
        "update `tab{0}` set lft=%s, rgt=%s, modified=%s where name=%s".format(doctype),
...
def update_move_node(doc, parent_field):
    ...
    if parent:
        ...
        max_rgt = frappe.db.sql(
            """select max(rgt) from `tab{0}`""".format(doc.doctype)
        )[0][0]
...
def rebuild_node(doctype, parent, left, parent_field):
    ...
    result = frappe.db.sql(
        "SELECT name FROM `tab{0}` WHERE `{1}`=%s".format(doctype, parent_field),
        (parent),
    )
    for r in result:
        right = rebuild_node(doctype, r[0], right, parent_field)
...
def validate_loop(doctype, name, lft, rgt):
    ...
    if name in frappe.db.sql_list(
        """select name from `tab{0}` where lft <= %s and rgt >= %s""".format(doctype),
        (lft, rgt),
    ):
        frappe.throw(
...
class NestedSet(Document):
    ...
    def before_rename(self, olddn, newdn, merge=False, group_fname="is_group"):
        if merge and hasattr(self, group_fname):
            ...
            if self.get(group_fname) != is_group:
                frappe.throw(
                    _(
                        "Merging is only possible between Group-to-Group or Leaf"
                        " Node-to-Leaf Node"
                    ),
    ...
    def after_rename(self, olddn, newdn, merge=False):
        ...
        frappe.db.sql(
            "update `tab{0}` set old_parent=%s where {1}=%s".format(
                self.doctype, parent_field
            ),
            (newdn, newdn),
        )
    ...
    def validate_one_root(self):
        if not self.get(self.nsm_parent_field):
            if self.get_root_node_count() > 1:
                frappe.throw(
                    _("""Multiple root nodes not allowed."""),
