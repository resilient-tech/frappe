...
class BackupGenerator:
    ...
    def take_dump(self):
        ...
        database_header_content = [
            f"Backup generated by Frappe {frappe.__version__} on branch"
            f" {get_app_branch('frappe') or 'N/A'}",
            "",
        ]
        ...
        if self.db_type == "postgres":
            if self.backup_includes:
                args["include"] = " ".join(
                    [
                        "--table='public.\"{0}\"'".format(table)
                        for table in self.backup_includes
                    ]
                )
            elif self.backup_excludes:
                args["exclude"] = " ".join(
            ...
            if self.backup_includes:
                args["include"] = " ".join(
                    ["'{0}'".format(x) for x in self.backup_includes]
                )
            elif self.backup_excludes:
                args["exclude"] = " ".join(
                    [
...
def get_backup():
    ...
    frappe.msgprint(
        _(
            "Download link for your backup will be emailed on the following email"
            " address: {0}"
