            ...
            all_contents.append(
                {
                    ...
                    "published": published,
                    "title": frappe.db.escape(
                        (title or "")[: int(frappe.db.VARCHAR_LEN)]
                    ),
                    "route": frappe.db.escape(
                        (route or "")[: int(frappe.db.VARCHAR_LEN)]
                    ),
                }
            )
    if all_contents:
...
def insert_values_for_multiple_docs(all_contents):
    ...
    for content in all_contents:
        values.append(
            "({doctype}, {name}, {content}, {published}, {title}, {route})".format(
                **content
            )
        )
...
def update_global_search(doc):
    ...
    for field in doc.meta.get_global_search_fields():
        if (
            ...
            and field.fieldtype not in frappe.model.table_fields
        ):
...
def search(text, start=0, limit=20, doctype=""):
    ...
    for text in set(text.split("&")):
        ...
        mariadb_fields = (
            "`doctype`, `name`, `content`, MATCH (`content`) AGAINST ({} IN BOOLEAN"
            " MODE) AS rank".format(mariadb_text)
        )
        postgres_fields = (
            '`doctype`, `name`, `content`, TO_TSVECTOR("content") @@'
            " PLAINTO_TSQUERY({}) AS rank".format(frappe.db.escape(text))
        )
