...
def main(
    ...
    if ui_tests:
        print(
            "Selenium testing has been deprecated\nUse bench --site {site_name}"
            " run-ui-tests for Cypress tests"
        )
    ...
    try:
        ...
        if doctype:
            ...
            ret = run_tests_for_module(
                module, verbose, tests, profile, junit_xml_output=junit_xml_output
            )
        else:
            ret = run_all_tests(
                app,
...
def run_tests_for_module(
    module, verbose=False, tests=(), profile=False, junit_xml_output=False
):
...
def _run_unittest(
    modules, verbose=False, tests=(), profile=False, junit_xml_output=False
):
    test_suite = unittest.TestSuite()
    ...
    if junit_xml_output:
        ...
        runner = unittest_runner(
            resultclass=TimeLoggingTestResult, verbosity=1 + (verbose and 1 or 0)
        )
