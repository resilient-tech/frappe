...
def check_database_settings():
    ...
    if not result:
        ...
        msg = (
            "Creation of your site - {x} failed because MariaDB is not properly"
            " {sep}configured.  If using version 10.2.x or earlier, make sure you use"
            " the {sep}the Barracuda storage engine. {sep}{sep}Please verify the"
            " settings above in MariaDB's my.cnf.  Restart MariaDB.  And {sep}then run"
            " `bench new-site {x}` again.{sep2}"
        ).format(x=site, sep2="\n" * 2, sep="\n")
        print_db_config(msg)
    return result
