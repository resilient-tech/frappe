...
def bootstrap_database(db_name, verbose, source_sql=None):
    ...
    if "tabDefaultValue" not in frappe.db.get_tables():
        ...
        secho(
            "Table 'tabDefaultValue' missing in the restored site. This may be due to"
            " incorrect permissions or the result of a restore from a bad backup file."
...
def import_db_from_sql(source_sql=None, verbose=False):
    ...
    _command = (
        f"psql {frappe.conf.db_name} -h {frappe.conf.db_host or 'localhost'} -p"
        f" {str(frappe.conf.db_port or '5432')} -U {frappe.conf.db_name}"
    )
