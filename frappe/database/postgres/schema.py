    ...
    def alter(self):
        ...
        for col in self.change_type:
            ...
            if col.fieldtype in ("Datetime"):
                ...
                using_clause = "USING {}::timestamp without time zone".format(
                    col.fieldname
                )
            ...
            query.append(
                "ALTER COLUMN `{0}` TYPE {1} {2}".format(
                    col.fieldname,
                    get_definition(
                        col.fieldtype, precision=col.precision, length=col.length
                    ),
                    using_clause,
                )
            )
        ...
        for col in self.set_default:
            ...
            query.append(
                "ALTER COLUMN `{}` SET DEFAULT {}".format(col.fieldname, col_default)
            )
        ...
        for col in self.drop_index:
            ...
            if col.fieldname != "name":
                ...
                if not frappe.db.has_index(self.table_name, col.fieldname):
                    drop_index_query += 'DROP INDEX IF EXISTS "{}" ;'.format(
                        col.fieldname
                    )
