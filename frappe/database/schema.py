            ...
            if "varchar" in frappe.db.type_map.get(col.fieldtype, ()):
                ...
                if cint(current_length) != cint(new_length):
                    ...
                    if (
                        max_length
                        and max_length[0][0]
                        and max_length[0][0] > new_length
                    ):
                        if col.fieldname in self.columns:
                            self.columns[col.fieldname].length = current_length
                        info_message = _(
                            "Reverting length to {0} for '{1}' in '{2}'. Setting the"
                            " length as {3} will cause truncation of data."
                        ).format(
...
class DbColumn:
    ...
    def build_for_alter_table(self, current_def):
        ...
        # unique
        if (self.unique and not current_def["unique"]) and column_type not in (
            "text",
            "longtext",
        ):
        ...
        if (current_def["index"] and not self.set_index and not self.unique) or (
            current_def["unique"] and not self.unique
        ):
