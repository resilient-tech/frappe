...
class Database(object):
        ...
        if query[:6].lower() in ("update", "insert", "delete"):
            ...
            if self.transaction_writes > 200000:
                if self.auto_commit_on_many_writes:
                    ...
                    frappe.throw(
                        _(
                            "Too many writes in one request. Please send smaller"
                            " requests"
                        ),
        ...
        def _build_condition(key):
            ...
            if isinstance(value, (list, tuple)):
                ...
                if isinstance(value[1], (tuple, list)):
                    ...
                    _rhs = " ({0})".format(
                        ", ".join([self.escape(v) for v in value[1]])
                    )
                    del values[key]
    ...
    def _get_values_from_table(
        ...
        r = self.sql(
            "select {fields} from `tab{doctype}` {where} {conditions} {order_by}"
            " {for_update}".format(
        ...
        **Warning:** this function will not call Document events and should be avoided in normal cases.
        ...
        if dn and dt != dn:
            ...
            for name in self.get_values(dt, dn, "name", for_update=for_update):
                ...
                self.sql(
                    """update `tab{0}`
					set {1} where name=%(name)s""".format(
                        dt, ", ".join(set_values)
                    ),
                    values,
                    debug=debug,
    ...
    def delete(self, doctype, conditions, debug=False):
        if conditions:
            ...
            return self.sql(
                "DELETE FROM `tab{doctype}` where {conditions}".format(
