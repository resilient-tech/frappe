...
def bulk_workflow_approval(docnames, doctype, action):
    ...
    for (idx, docname) in enumerate(docnames, 1):
        ...
        try:
            ...
            frappe.log_error(
                ...
                "Workflow {0} threw an error for {1} {2}".format(
                    action, doctype, docname
                ),
            )
        finally:
            if not message_dict:
                if frappe.message_log:
                    ...
                    successful_transactions[docname].append(
                        {"docname": docname, "message": None}
                    )
    ...
    print_workflow_log(
        failed_transactions, _("Failed Transactions"), doctype, indicator
    )
    print_workflow_log(
                ...
                for log in messages[doc]:
                    if log.get("message"):
                        html += (
                            ...
                            " style='padding:2.5px'>{0}</div>".format(
                                log.get("message")
                            )
                        )
                html += "</details>"
    ...
    try:
        for (i, doc) in enumerate(docs, 1):
            ...
            if not actions:
                return []
            common_actions = (
                actions if i == 1 else set(common_actions).intersection(actions)
            )
