    ...
    for name in names or []:
        ...
        if doctype == "DocType":
            if for_reload:
                ...
                frappe.db.sql(
                    "delete from `tabProperty Setter` where doc_type = %s", name
                )
        ...
        if doc and not for_reload:
            ...
            frappe.defaults.clear_default(
                parenttype="User Permission", key=doctype, value=name
            )
...
def delete_from_table(doctype, name, ignore_doctypes, doc):
    ...
    for t in list(set(tables)):
        if t not in ignore_doctypes:
            frappe.db.sql(
                "delete from `tab%s` where parenttype=%s and parent = %s"
                % (t, "%s", "%s"),
                (doctype, name),
            )
...
def check_permission_and_not_submitted(doc):
    ...
    if doc.docstatus == 1:
        frappe.msgprint(
            _(
                "{0} {1}: Submitted Record cannot be deleted. You must {2} Cancel {3}"
                " it first."
            ).format(
                _(doc.doctype),
                doc.name,
                "<a href='https://docs.erpnext.com//docs/user/manual/en/setting-up/articles/delete-submitted-document'"
                " target='_blank'>",
                "</a>",
            ),
...
def check_if_doc_is_linked(doc, method="Delete"):
    ...
    link_fields = [
        [lf["parent"], lf["fieldname"], lf["issingle"]] for lf in link_fields
    ]
        ...
        if meta.issingle:
            ...
            for refdoc in frappe.db.sql(
                ...
                if (method == "Delete" and refdoc.docstatus < 2) or (
                    ...
                    at_position = (
                        "at Row: {0}".format(refdoc.idx) if meta.istable else ""
                    )
