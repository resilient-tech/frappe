...
def make_autoname(key="", doctype="", doc=""):
    ...
    *Example:*
    ...
    if "#" not in key:
        ...
        if doctype:
            error_message = _("Invalid naming series (. missing) for {0}").format(
                doctype
            )
...
def validate_name(doctype, name, case=None, merge=False):
    ...
    if name.startswith("New " + doctype):
        frappe.throw(
            _(
                "There were some errors setting the name, please contact the"
                " administrator"
            ),
            frappe.NameError,
        )
...
def append_number_if_name_exists(
    ...
    regex = "^{value}{separator}\d+$".format(
        value=re.escape(value), separator=separator
    )
