...
class BaseDocument(object):
    ...
    def get(self, key=None, filters=None, limit=None, default=None):
        if key:
            ...
            if (
                value is None
                and key not in self.ignore_in_getter
    ...
    def append(self, key, value=None):
        ...
        if isinstance(value, (dict, BaseDocument)):
            ...
            raise ValueError(
                'Document for field "{0}" attached to child table of "{1}" must be a'
                " dict or BaseDocument, not {2} ({3})".format(
    ...
    def get_valid_dict(
        self, sanitize=True, convert_dates_to_str=False, ignore_nulls=False
    ):
        d = frappe._dict()
        for fieldname in self.meta.get_valid_columns():
            ...
            if df:
                if df.fieldtype == "Check":
                    ...
                    and not isinstance(d[fieldname], float)
                ):
                    d[fieldname] = flt(d[fieldname])

                elif (
                    df.fieldtype in ("Datetime", "Date", "Time") and d[fieldname] == ""
                ):
                    d[fieldname] = None
            ...
            if convert_dates_to_str and isinstance(
                d[fieldname], (datetime.datetime, datetime.time, datetime.timedelta)
            ):
    ...
    def as_dict(
        self, no_nulls=False, no_default_fields=False, convert_dates_to_str=False
    ):
        ...
        for df in self.meta.get_table_fields():
            ...
            doc[df.fieldname] = [
                d.as_dict(convert_dates_to_str=convert_dates_to_str, no_nulls=no_nulls)
                for d in children
            ]
    ...
    def get_table_field_doctype(self, fieldname):
        try:
            ...
            if self.doctype == "DocType":
                return dict(links="DocType Link", actions="DocType Action").get(
                    fieldname
                )
            raise
    ...
    def get_formatted(
        self, fieldname, doc=None, currency=None, absolute_value=False, translated=False
    ):
