...
class CustomField(Document):
    ...
    def set_fieldname(self):
        if not self.fieldname:
            ...
            self.fieldname = "".join(
                filter(
                    lambda x: x.isdigit() or x.isalpha() or "_",
    ...
    def before_insert(self):
        ...
        if self.fieldname in fieldnames:
            frappe.throw(
                _("A field with the name '{}' already exists in doctype {}.").format(
                    self.fieldname, self.dt
                )
            )
    ...
    def on_trash(self):
        ...
        if self.owner == "Administrator" and frappe.session.user != "Administrator":
            frappe.throw(
                _(
                    "Custom Field {0} is created by the Administrator and can only be"
                    " deleted through the Administrator account."
    ...
    def validate_insert_after(self, meta):
        if not meta.get_field(self.insert_after):
            frappe.throw(
                _(
                    "Insert After field '{0}' mentioned in Custom Field '{1}', with"
                    " label '{2}', does not exist"
