...
class CustomizeForm(Document):
    ...
    def validate_doctype(self, meta):
        ...
        if meta.custom:
            frappe.throw(
                _(
                    "Only standard DocTypes are allowed to be customized from Customize"
                    " Form."
                )
            )
    ...
    def allow_property_change(self, prop, meta_df, df):
        if prop == "fieldtype":
            ...
            if not frappe.db.get_value(
                ...
                frappe.msgprint(
                    _(
                        "Row {0}: Not allowed to enable Allow on Submit for standard"
                        " fields"
                    ).format(df.idx)
                )
                return False
    ...
    def validate_fieldtype_length(self):
        for field in self.check_length_for_fieldtypes:
            ...
            if docs:
                frappe.throw(
                    _(
                        "Value for field {0} is too long in {1}. Length should be"
                        " lesser than {2} characters"
