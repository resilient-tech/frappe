...
class TestEventProducer(unittest.TestCase):
    ...
    def test_child_table_sync_with_dependencies(self):
        ...
        producer_note = frappe._dict(
            doctype="Note",
            title="test child table dependency sync",
    ...
    def test_dynamic_link_dependencies_synced(self):
        ...
        self.assertEqual(
            producer_link_doc.name,
            frappe.db.get_value("ToDo", producer_doc.name, "reference_name"),
        )
    ...
    def test_naming_configuration(self):
        ...
        self.assertTrue(
            frappe.db.exists(
                "ToDo",
                {"remote_docname": producer_doc.name, "remote_site_name": producer_url},
            )
        )
    ...
    def test_conditional_events(self):
        ...
        producer_note1 = frappe._dict(
            doctype="Note", public=0, title="test conditional sync"
        )
        delete_on_remote_if_exists(producer, "Note", {"title": producer_note1["title"]})
        producer_note1 = producer.insert(producer_note1)
    ...
    def test_event_sync_log(self):
        ...
        self.pull_producer_data()
        self.assertTrue(
            frappe.db.exists("Event Sync Log", {"docname": producer_doc.name})
        )
    ...
    def test_mapping(self):
        ...
        # check inserted
        self.assertTrue(
            frappe.db.exists("ToDo", {"description": producer_note.content})
        )
...
def create_event_producer(producer_url):
    ...
    event_producer.append(
        "producer_doctypes", {"ref_doctype": "Note", "use_same_name": 1}
    )
