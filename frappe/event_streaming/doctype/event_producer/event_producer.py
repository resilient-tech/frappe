        ...
        if not self.incoming_change:
            if frappe.db.exists("Event Producer", self.name):
                if not self.api_key or not self.api_secret:
                    frappe.throw(
                        _(
                            "Please set API Key and Secret on the producer and consumer"
                            " sites first."
                        )
                    )
                else:
        ...
        for entry in self.producer_doctypes:
            if not entry.use_same_name:
                if not frappe.db.exists(
                    "Custom Field",
                    {"fieldname": "remote_docname", "dt": entry.ref_doctype},
                ):
                    df = dict(
                ...
                if not frappe.db.exists(
                    "Custom Field",
                    {"fieldname": "remote_site_name", "dt": entry.ref_doctype},
                ):
                    df = dict(
    ...
    try:
        if not update.use_same_name:
            return frappe.get_doc(
                update.ref_doctype, {"remote_docname": update.docname}
            )
    ...
    def sync_child_table_dependencies(doc, table_fields, producer_site):
        for df in table_fields:
            ...
            for entry in child_table:
                ...
                set_dependencies(
                    child_doc,
                    frappe.get_meta(entry.doctype).get_link_fields(),
                    producer_site,
                )
...
def new_event_notification(producer_url):
    ...
    if not jobs or enqueued_method not in jobs[frappe.local.site]:
        frappe.enqueue(
            enqueued_method, queue="default", **{"event_producer": producer_url}
        )
