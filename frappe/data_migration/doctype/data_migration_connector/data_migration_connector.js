...
frappe.ui.form.on("Data Migration Connector", {
	onload(frm) {
		if (frappe.boot.developer_mode) {
			frm.add_custom_button(__("New Connection"), () =>
				frm.events.new_connection(frm)
			);
		}
	},
	new_connection(frm) {
		const d = new frappe.ui.Dialog({
			...
			fields: [
				...
				},
				{
					...
					fieldtype: "Data",
					description: "For e.g: Shopify Connection",
					reqd: 1,
				},
			],
				...
				frm.events
					...
					.then((r) => {
						if (r.message) {
							...
							frm.save();
							frappe.show_alert(
								__("New module created {0}", [r.message])
							);
							d.hide();
						}
					});
	...
	create_new_connection(module, connection_name) {
		return frappe.call(
			"frappe.data_migration.doctype.data_migration_connector.data_migration_connector.create_new_connection",
			{
				module,
				connection_name,
			}
