    ...
    if not asset_key in translation_assets:
        if fortype == "doctype":
            ...
            messages += frappe.db.sql(
                "select 'Print Format:', name from `tabPrint Format`"
            )
            messages += frappe.db.sql("select 'DocType:', name from tabDocType")
            messages += frappe.db.sql("select 'Role:', name from tabRole")
            messages += frappe.db.sql("select 'Module:', name from `tabModule Def`")
            messages += frappe.db.sql(
                "select '', format from `tabWorkspace Shortcut` where format is not"
                " null"
            )
        ...
        cache.hset(
            "translation_assets", frappe.local.lang, translation_assets, shared=True
        )
...
def get_full_dict(lang):
    ...
    ) and frappe.local.lang_full_dict.get(lang, None):
...
def load_lang(lang, apps=None):
    ...
    if not out:
        ...
        for app in apps or frappe.get_all_apps(True):
            path = os.path.join(
                frappe.get_pymodule_path(app), "translations", lang + ".csv"
            )
...
def get_messages_from_workflow(doctype=None, app_name=None):
    ...
    for w in workflows:
        ...
        states = frappe.db.sql(
            "select distinct message from `tabWorkflow Document State` where parent=%s"
...
def get_messages_from_include_files(app_name=None):
    ...
    messages = []
    for file in (frappe.get_hooks("app_include_js", app_name=app_name) or []) + (
        frappe.get_hooks("web_include_js", app_name=app_name) or []
    ):
        messages.extend(
            get_messages_from_file(os.path.join(frappe.local.sites_path, file))
        )
...
def get_messages_from_file(path):
    ...
    if os.path.exists(path):
        with open(path, "r") as sourcefile:
            data = [
                (os.path.relpath(path, apps_path), message, context, line)
                for line, message, context in extract_messages_from_code(
                    sourcefile.read()
                )
            ]
            return data
...
def write_translations_file(app, lang, full_dict=None, app_messages=None):
    ...
    write_csv_file(
        ...
        app_messages,
        full_dict or get_full_dict(lang),
    )
...
def rename_language(old_name, new_name):
    ...
    language_in_system_settings = frappe.db.get_single_value(
...
def get_source_additional_info(source, language=""):
    ...
    return translator.post_api(
        "translator.api.get_source_additional_info", params=locals()
    )
