		...
		return new Promise((resolve) => {
			...
			if (doc.doc_type) {
				frappe.model.with_doctype(doc.doc_type, function () {
					var fields = $.map(
						frappe.get_doc("DocType", frm.doc.doc_type).fields,
						function (d) {
							if (
								frappe.model.no_value_type.indexOf(
									d.fieldtype
								) === -1 ||
								d.fieldtype === "Table"
							) {
					...
					frappe.meta.get_docfield(
						...
						"amount_field",
						frm.doc.name
					).options = [""].concat(currency_fields);
		...
		frm.add_custom_button(__("Get Fields"), () => {
			...
			frappe.model.with_doctype(frm.doc.doc_type, () => {
				...
				for (let field of meta.fields) {
					if (
						webform_fieldtypes.includes(field.fieldtype) &&
...
frappe.ui.form.on("Web Form Field", {
	...
	fieldname: function (frm, doctype, name) {
		...
		doc.fieldtype =
			frappe.meta
				.get_docfield("Web Form Field", "fieldtype")
				.options.split("\n")
