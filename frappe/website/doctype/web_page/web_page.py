    ...
    def validate_dates(self):
        if self.end_date:
            ...
            if self.published and now() > self.end_date:
                ...
                frappe.msgprint(
                    _(
                        "Clearing end date, as it cannot be in the past for published"
                        " pages."
                    )
                )
...
def check_publish_status():
    ...
    for page in web_pages:
        ...
        if page.published:
            ...
            if (start_date and now_date < start_date) or (
                end_date and now_date > end_date
            ):
...
def check_broken_links():
    ...
    for p in frappe.db.sql(
        "select name, main_section from `tabWeb Page`", as_dict=True
    ):
