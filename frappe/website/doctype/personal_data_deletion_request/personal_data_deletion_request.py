...
class PersonalDataDeletionRequest(Document):
    ...
    def send_verification_mail(self):
        ...
        url = (
            frappe.utils.get_url(
                "/api/method/frappe.website.doctype.personal_data_deletion_request.personal_data_deletion_request.confirm_deletion"
            )
            + "?"
            + get_signed_params(
                {"email": self.email, "name": self.name, "host_name": host_name}
            )
        )
    ...
    def anonymize_data(self):
        ...
        anonymize_value_map = {"Date": "1111-01-01", "Int": 0, "Code": "http://xxxxx"}
        ...
        for ref_doc in privacy_docs:
            ...
            if ref_doc.get(
                "applies_to_website_user"
            ) and "Guest" not in frappe.get_roles(self.email):
                continue
            ...
            for field in personal_fields:
                ...
                field_value = (
                    anonymize_value_map.get(field_details.fieldtype, str(field))
                    if not field_details.unique
            ...
            docs = frappe.get_all(
                ref_doc["doctype"],
                {ref_doc["match_field"]: ("like", "%" + self.email + "%")},
                ["name", ref_doc["match_field"]],
            )
            for d in docs:
                if not re.search(regex, d[ref_doc["match_field"]]):
                    continue

                anonymize_match_value = ", ".join(
                    map(
                        lambda x: self.name if re.search(regex, x) else x,
                        d[ref_doc["match_field"]].split(),
                    )
                )
                frappe.db.sql(
                    """UPDATE `tab{0}`
