frappe.ui.form.on("Website Settings", {
	...
	set_parent_label_options: function (frm) {
		frappe.meta.get_docfield(
			...
			"parent_label",
			frm.docname
		).options = frm.events.get_parent_options(frm, "top_bar_items");

		if (
			$(frm.fields_dict.top_bar_items.grid.wrapper).find(".grid-row-open")
		) {
	...
	set_parent_label_options_footer: function (frm) {
		frappe.meta.get_docfield(
			...
			"parent_label",
			frm.docname
		).options = frm.events.get_parent_options(frm, "footer_items");

		if (
			$(frm.fields_dict.footer_items.grid.wrapper).find(".grid-row-open")
		) {
