...
@click.option(
    ...
    default=False,
    help=(
        "Ignore the validations and downgrade warnings. This action is not recommended"
    ),
)
@pass_context
def restore(
    ...
    if is_partial(decompressed_file_name):
        click.secho(
            "Partial Backup file detected. You cannot use a partial file to restore a"
            " Frappe Site.",
            fg="red",
        )
        click.secho(
            "Use `bench partial-restore` to restore a partial backup to an existing"
            " site.",
            fg="yellow",
        )
        sys.exit(1)
    ...
    for site in context.sites:
        ...
        for app in apps:
            try:
                ...
                print(
                    "App {} is Incompatible with Site {}{}".format(app, site, err_msg)
                )
                exit_code = 1
...
def add_to_hosts(context):
    ...
    for site in context.sites:
        frappe.commands.popen(
            "echo 127.0.0.1\t{0} | sudo tee -a /etc/hosts".format(site)
        )
    if not context.sites:
        raise SiteNotSpecifiedError
...
def backup(
    ...
    for site in context.sites:
        try:
            ...
            click.secho(
                "Backup failed for Site {0}. Database or site_config.json may be"
                " corrupted".format(site),
                fg="red",
            )
            if verbose:
...
def _drop_site(
    ...
    try:
        ...
        if force:
            ...
            messages = [
                "=" * 80,
                "Error: The operation has stopped because backup of {0}'s database"
                " failed.".format(site),
                "Reason: {0}\n".format(str(err)),
                "Fix the issue and try again.",
                "Hint: Use 'bench drop-site {0} --force' to force the removal of {0}"
                .format(site),
            ]
            click.echo("\n".join(messages))
            sys.exit(1)
