...
@click.option(
    ...
    is_flag=True,
    default=False,
    help="Copy the files instead of symlinking with force",
)
...
def jupyter(context):
    ...
    try:
        ...
        print(
            "Creating folder to keep jupyter notebooks at {}".format(
...
@click.option(
    "--doctype-list-path",
    help=(
        "Path to .txt file for list of doctypes. Example"
        " erpnext/tests/server/agriculture.txt"
    ),
)
...
def run_tests(
    ...
    if not (allow_tests or os.environ.get("CI")):
        ...
        click.secho(
            "bench --site {0} set-config allow_tests true".format(site), fg="green"
        )
        return
...
def run_ui_tests(context, app, headless=False):
    ...
    password_env = (
        "CYPRESS_adminPassword={}".format(admin_password) if admin_password else ""
    )
    ...
    if not (
        os.path.exists(cypress_path) or os.path.exists(plugin_path)
    ...
    run_or_open = (
        "run --browser firefox --record --key 4a48f41c-11b3-425b-aa88-c58048fa69eb"
        if headless
        else "open"
    )
