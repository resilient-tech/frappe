...
def _is_scheduler_enabled():
    ...
    try:
        frappe.connect()
        enable_scheduler = (
            cint(frappe.db.get_single_value("System Settings", "enable_scheduler"))
            and True
            or False
        )
    except:
...
@click.option(
    ...
    default=None,
    help=(
        'one of "all", "weekly", "monthly", "hourly", "daily", "weekly_long",'
        ' "daily_long"'
    ),
)
...
def ready_for_migration(context, site=None):
    ...
    try:
        ...
        if pending_jobs:
            ...
            print(
                "READY for migration: site {0} does not have any background jobs"
                .format(site)
            )
            return 0
