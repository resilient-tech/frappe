...
class BackgroundJobs {
	constructor(wrapper) {
		...
		this.show_failed_button = this.page.add_inner_button(
			__("Show Failed Jobs"),
			() => {
		...
		this.remove_failed_button = this.page.add_inner_button(
		...
		$(frappe.render_template("background_jobs_outer")).appendTo(
			this.page.body
		);
	...
	refresh_jobs() {
		...
		frappe.call({
			...
			callback: (res) => {
				...
				$(
					frappe.render_template("background_jobs", {
						jobs: res.message || [],
					})
