...
frappe.ui.form.on("Data Import Legacy", {
	...
	refresh: function (frm) {
		...
		if (
			frm.doc.action == "Insert new records" ||
			frm.doc.action == "Update records"
		) {
		...
		if (
			...
			frm.doc.total_rows &&
			frm.doc.docstatus === 0 &&
			(!frm.doc.import_status || frm.doc.import_status == "Failed")
		) {
			frm.page
				.set_primary_action(__("Start Import"), function () {
					frappe.call({
						btn: frm.page.btn_primary,
						method:
							"frappe.core.doctype.data_import_legacy.data_import_legacy.import_data",
						args: {
...
frappe.data_import.download_dialog = function (frm) {
	...
	const get_fields = (dt) =>
		frappe.meta.get_docfields(dt).filter(filter_fields);

	const get_doctype_checkbox_fields = () => {
		return dialog.fields
			.filter((df) => df.fieldname.endsWith("_fields"))
			.map((df) => dialog.fields_dict[df.fieldname]);
	};

	const doctype_fields = get_fields(frm.doc.reference_doctype).map((df) => {
				...
				if (this.value == "Mandatory" || this.value == "Manually") {
					...
					fields.map((multicheck_field) => {
						multicheck_field.options.map((option) => {
							...
							$(multicheck_field.$wrapper)
								.find(`:checkbox[data-unit="${option.value}"]`)
								.prop("checked", false)
								.trigger("click");
						});
					});
					...
					$(dialog.body)
						.find(`[data-fieldtype="MultiCheck"] :checkbox`)
						.prop("disabled", true);
				}
			},
		},
	...
	const child_table_fields = frappe.meta
		.get_table_fields(frm.doc.reference_doctype)
		.map((df) => {
			return {
				label: df.options,
			...
			if (frm.doc.reference_doctype) {
				var export_params = () => {
					...
					if (values.select_columns) {
						columns = get_doctype_checkbox_fields().reduce(
							(columns, field) => {
	...
	$(dialog.body)
		.find(
			'div[data-fieldname="select_all"], div[data-fieldname="unselect_all"]'
		)
