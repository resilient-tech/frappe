...
class File(Document):
                ...
                _("Added {0}").format(
                    "<a href='{file_url}' target='_blank'>{file_name}</a>{icon}".format(
                        **{
                            ...
                            "file_url": quote(frappe.safe_encode(self.file_url))
                            if self.file_url
                            else self.file_name,
    ...
    def get_successor(self):
        return frappe.db.get_values(
            doctype="File", filters={"folder": self.name}, fieldname="name"
        )
    ...
    def validate_folder(self):
        if (
            not self.is_home_folder
            and not self.folder
            and not self.flags.ignore_folder_validate
        ):
            self.folder = "Home"
    ...
    def check_folder_is_empty(self):
        ...
        files = frappe.get_all(
            "File", filters={"folder": self.name}, fields=("name", "file_name")
        )
    ...
    def get_file_url(self):
        data = frappe.db.get_value(
            "File", self.file_data_name, ["file_name", "file_url"], as_dict=True
        )
    ...
    def write_file(self):
        ...
        with open(
            os.path.join(file_path.encode("utf-8"), self.file_name.encode("utf-8")),
            "wb+",
        ) as f:
    ...
    def get_file_doc(self):
        ...
        if self.file_url is None:
            self.file_url = r.file_url
        if self.file_name is None:
            self.file_name = r.file_name
        if self.attached_to_doctype is None:
            self.attached_to_doctype = r.doctype
        if self.attached_to_name is None:
            self.attached_to_name = r.docname
        if self.attached_to_field is None:
            self.attached_to_field = r.docfield
    ...
    def validate_url(self, df=None):
        if self.file_url:
            if not self.file_url.startswith(
                ("http://", "https://", "/files/", "/private/files/")
            ):
            ...
            if not self.file_url.startswith(("http://", "https://")):
                ...
                if not os.path.commonpath([root_files_path]) == os.path.commonpath(
                    [root_files_path, self.get_full_path()]
                ):
    ...
    def save_file(self, content=None, decode=False, ignore_existing_file_check=False):
        ...
        if os.path.exists(
            encode(get_files_path(self.file_name, is_private=self.is_private))
        ):
    ...
    def get_file_data_from_hash(self):
        for name in frappe.db.sql_list(
            "select name from `tabFile` where content_hash=%s and is_private=%s",
...
def delete_file(path):
    ...
    if path:
        if ".." in path.split("/"):
            frappe.msgprint(
                _(
                    "It is risky to delete this file: {0}. Please contact your System"
                    " Manager."
                ).format(path)
            )
...
def remove_file(
    fid=None, attached_to_doctype=None, attached_to_name=None, from_delete=False
):
    ...
    if attached_to_doctype and attached_to_name and not from_delete:
        ...
        comment = doc.add_comment(
            "Attachment Removed", _("Removed {0}").format(file_name)
        )
        ...
        try:
            ...
            if ptype in ["write", "create", "delete"]:
                ...
                if ptype == "delete" and not has_access:
                    frappe.throw(
                        _(
                            "Cannot delete file as it belongs to {0} {1} for which you"
...
def extract_images_from_html(doc, content):
    ...
    if content and isinstance(content, string_types):
        content = re.sub(
            "<img[^>]*src\s*=\s*[\"'](?=data:)(.*?)[\"']", _save_file, content
        )
