...
class DocType(Document):
    ...
    def validate(self):
        ...
        if self.default_print_format and not self.custom:
            frappe.throw(
                _(
                    "Standard DocType cannot have default print format, use Customize"
                    " Form"
                )
            )
    ...
    def check_developer_mode(self):
        ...
        if not frappe.conf.get("developer_mode") and not self.custom:
            frappe.throw(
                _(
                    "Not in Developer Mode! Set in site_config.json or make 'Custom'"
                    " DocType."
                ),
    ...
    def setup_fields_to_fetch(self):
        ...
        if set(old_fields_to_fetch) != set(
            [df.fieldname for df in new_meta.get_fields_to_fetch()]
        ):
    ...
    def validate_website(self):
        ...
        if self.has_web_view:
            ...
            if not "route" in [d.fieldname for d in self.fields]:
                frappe.throw(
                    _('Field "route" is mandatory for Web Views'), title="Missing Field"
                )
    ...
    def change_modified_of_parent(self):
        ...
        for p in parent_list:
            frappe.db.sql(
                "UPDATE `tabDocType` SET modified=%s WHERE `name`=%s", (now(), p.parent)
            )
        ...
        for d in self.get("fields"):
            if d.fieldtype:
                if not getattr(d, "fieldname", None):
                    if d.label:
                        d.fieldname = (
                            d.label.strip().lower().replace(" ", "_").strip("?")
                        )
                        ...
                        d.fieldname = (
                            d.fieldtype.lower().replace(" ", "_") + "_" + str(d.idx)
                        )
                else:
                    if d.fieldname in restricted:
                        frappe.throw(
                            _("Fieldname {0} is restricted").format(d.fieldname),
    ...
    def validate_series(self, autoname=None, name=None):
        ...
        elif self.autoname == "naming_series:" and not self.get(
            "fields", {"fieldname": "naming_series"}
        ):
        ...
        if (
            ...
            if used_in:
                frappe.throw(
                    _("Series {0} already used in {1}").format(prefix, used_in[0][0])
                )
    ...
    def sync_global_search(self):
        ...
        global_search_fields_after_update = [
            d.fieldname for d in self.fields if d.in_global_search
        ]
        if self.show_name_in_global_search:
            global_search_fields_after_update.append("name")

        if set(global_search_fields_before_update) != set(
            global_search_fields_after_update
        ):
            now = (
                (not frappe.request) or frappe.flags.in_test or frappe.flags.in_install
            )
            frappe.enqueue(
    ...
    def set_base_class_for_controller(self):
        ...
        controller_path = frappe.get_module_path(
            frappe.scrub(self.module),
            "doctype",
            frappe.scrub(self.name),
            frappe.scrub(self.name) + ".py",
        )
        ...
        if (
            "\nfrom frappe.model.document import Document" in code
            and class_string in code
        ):
            code = code.replace(
                "from frappe.model.document import Document",
    ...
    def after_rename(self, old, new, merge=False):
        ...
        if self.issingle:
            frappe.db.sql(
                """update tabSingles set doctype=%s where doctype=%s""", (new, old)
            )
            frappe.db.sql(
        ...
        for fname in os.listdir(new_path):
            if frappe.scrub(old) in fname:
                ...
                new_file_name = os.path.join(
                    new_path, fname.replace(frappe.scrub(old), frappe.scrub(new))
                )
                shutil.move(old_file_name, new_file_name)

        self.rename_inside_controller(new, old, new_path)
        frappe.msgprint(
            _("Renamed files and replaced code in controllers, please check!")
        )
                ...
                with open(fname, "w") as f:
                    if fname.endswith(".js"):
                        file_content = code.replace(
                            old, new
                        )  # replace str with full str (js controllers)
            ...
            try:
                ...
                if old_field_names:
                    ...
                    for fieldname in old_field_names:
                        field_dict = list(
                            filter(
                                lambda d: d["fieldname"] == fieldname, docdict["fields"]
                            )
                        )
                        if field_dict:
    ...
    def prepare_for_import(docdict):
        ...
        if docdict.get("field_order"):
            ...
            for fieldname in docdict.get("field_order"):
                field_dict = list(
                    filter(
                        lambda d: d["fieldname"] == fieldname, docdict.get("fields", [])
                    )
                )
                if field_dict:
                    new_field_dicts.append(field_dict[0])
    ...
    def validate_nestedset(self):
        ...
        if not self.nsm_parent_field:
            ...
            frappe.throw(
                _("{0} is a mandatory field").format(field_label), frappe.MandatoryError
            )
        ...
        if self.nsm_parent_field and self.nsm_parent_field not in fieldnames:
            frappe.throw(
                _("Parent Field must be a valid fieldname"), InvalidFieldNameError
            )
    ...
    def add_nestedset_fields(self):
        ...
        self.append(
            "fields",
            {"label": "Is Group", "fieldtype": "Check", "fieldname": "is_group"},
        )
        self.append(
            "fields",
            {
    ...
    def get_max_idx(self):
        ...
        max_idx = frappe.db.sql(
            """select max(idx) from `tabDocField` where parent = %s""", self.name
        )
    ...
    def validate_name(self, name=None):
        ...
        if re.search("^[ \t\n\r]+|[ \t\n\r]+$", name, **flags):
            frappe.throw(
                _("DocType's name should not start or end with whitespace"),
                frappe.NameError,
            )
        ...
        if not re.match("^(?![\W])[^\d_\s][\w ]+$", name, **flags):
            frappe.throw(
                _(
                    "DocType's name should start with a letter and it can only consist"
                    " of letters, numbers, spaces and underscores"
                ),
                frappe.NameError,
            )
...
def validate_links_table_fieldnames(meta):
    ...
    for index, link in enumerate(meta.links):
        ...
        if not link_meta.get_field(link.link_fieldname):
            message = _("Row #{0}: Could not find field {1} in {2} DocType").format(
                index + 1,
                frappe.bold(link.link_fieldname),
                frappe.bold(link.link_doctype),
            )
...
def validate_fields(meta):
    ...
    def check_unique_fieldname(docname, fieldname):
        duplicates = list(
            filter(
                None,
                map(
                    lambda df: df.fieldname == fieldname and str(df.idx) or None, fields
                ),
            )
        )
        if len(duplicates) > 1:
    ...
    def check_illegal_mandatory(docname, d):
        if (
            (d.fieldtype in no_value_fields)
            and d.fieldtype not in table_fields
            and d.reqd
        ):
            frappe.throw(
                _("{0}: Field {1} of type {2} cannot be mandatory").format(
                    docname, d.label, d.fieldtype
                ),
    ...
    def check_link_table_options(docname, d):
        ...
        if d.fieldtype in ("Link",) + table_fields:
            if not d.options:
                frappe.throw(
                    _(
                        "{0}: Options required for Link or Table type field {1} in"
                        " row {2}"
                    ).format(docname, d.label, d.idx),
                    DoctypeLinkError,
                )
            ...
            if d.options != d.parent:
                ...
                if not options:
                    frappe.throw(
                        _(
                            "{0}: Options must be a valid DocType for field {1} in"
                            " row {2}"
                        ).format(docname, d.label, d.idx),
                    ...
                    frappe.throw(
                        _(
                            "{0}: Options {1} must be the same as doctype name {2} for"
                            " the field {3}"
    ...
    def check_hidden_and_mandatory(docname, d):
        if d.hidden and d.reqd and not d.default:
            frappe.throw(
                _(
                    "{0}: Field {1} in row {2} cannot be hidden and mandatory without"
                    " default"
                ).format(docname, d.label, d.idx),
    ...
    def check_width(d):
        if d.fieldtype == "Currency" and cint(d.width) < 100:
            frappe.throw(
                _("Max width for type Currency is 100px in row {0}").format(d.idx)
            )
    ...
    def check_in_list_view(is_table, d):
        if d.in_list_view and (d.fieldtype in not_allowed_in_list_view):
            ...
            frappe.throw(
                _("'{0}' not allowed for type {1} in row {2}").format(
                    property_label, d.fieldtype, d.idx
                )
            )
    ...
    def check_in_global_search(d):
        if d.in_global_search and d.fieldtype in no_value_fields:
            frappe.throw(
                _("'In Global Search' not allowed for type {0} in row {1}").format(
                    d.fieldtype, d.idx
                )
            )
    ...
    def check_dynamic_link_options(d):
        if d.fieldtype == "Dynamic Link":
            ...
            if (
                ...
                or (
                    doctype_pointer[0].fieldtype == "Link"
                    and doctype_pointer[0].options != "DocType"
                )
            ):
                frappe.throw(
                    _(
                        "Options 'Dynamic Link' type of field must point to another"
                        " Link Field with options as 'DocType'"
                    )
                )
    ...
    def check_illegal_default(d):
        ...
        if d.fieldtype == "Check" and cint(d.default) not in (0, 1):
            frappe.throw(
                ...
                ).format(frappe.bold(d.fieldname))
            )
    ...
    def check_unique_and_text(docname, d):
        ...
        if getattr(d, "unique", False):
            if d.fieldtype not in ("Data", "Link", "Read Only"):
                frappe.throw(
                    _("{0}: Fieldtype {1} for {2} cannot be unique").format(
                        docname, d.fieldtype, d.label
                    ),
                    NonUniqueError,
                )
            ...
            if not d.get("__islocal") and frappe.db.has_column(d.parent, d.fieldname):
                ...
                if has_non_unique_values and has_non_unique_values[0][0]:
                    frappe.throw(
                        _(
                            "{0}: Field '{1}' cannot be set as Unique as it has"
                            " non-unique values"
                        ).format(docname, d.label),
                        NonUniqueError,
                    )

        if d.search_index and d.fieldtype in (
            "Text",
            "Long Text",
            "Small Text",
            "Code",
            ...
            frappe.throw(
                _("{0}:Fieldtype {1} for {2} cannot be indexed").format(
                    docname, d.fieldtype, d.label
                ),
                CannotIndexedError,
            )
    ...
    def check_search_fields(meta, fields):
        ...
        search_fields = [
            field.strip() for field in (meta.search_fields or "").split(",")
        ]
        fieldtype_mapper = {
    ...
    def check_title_field(meta):
        ...
        if meta.title_field not in fieldname_list:
            frappe.throw(
                _("Title field must be a valid fieldname"), InvalidFieldNameError
            )
        ...
        def _validate_title_field_pattern(pattern):
            ...
            for fieldname in re.findall("{(.*?)}", pattern, re.UNICODE):
                ...
                if fieldname not in fieldname_list:
                    frappe.throw(
                        _(
                            "{{{0}}} is not a valid fieldname pattern. It should be"
                            " {{field_name}}."
                        ).format(fieldname),
                        InvalidFieldNameError,
                    )
    ...
    def check_image_field(meta):
        ...
        if not df:
            frappe.throw(
                _("Image field must be a valid fieldname"), InvalidFieldNameError
            )
        if df[0].fieldtype != "Attach Image":
            frappe.throw(
                _("Image field must be of type Attach Image"), InvalidFieldNameError
            )
    ...
    def check_is_published_field(meta):
        ...
        if meta.is_published_field not in fieldname_list:
            frappe.throw(
                _("Is Published Field must be a valid fieldname"), InvalidFieldNameError
            )
    ...
    def check_timeline_field(meta):
        ...
        if meta.timeline_field not in fieldname_list:
            frappe.throw(
                _("Timeline field must be a valid fieldname"), InvalidFieldNameError
            )

        df = meta.get("fields", {"fieldname": meta.timeline_field})[0]
        if df.fieldtype not in ("Link", "Dynamic Link"):
            frappe.throw(
                _("Timeline field must be a Link or Dynamic Link"),
                InvalidFieldNameError,
            )
        ...
        if meta.sort_field:
            ...
            for fieldname in sort_fields:
                if not fieldname in fieldname_list + list(default_fields):
                    frappe.throw(
                        _("Sort field {0} must be a valid fieldname").format(fieldname),
                        InvalidFieldNameError,
                    )
    ...
    def check_illegal_depends_on_conditions(docfield):
        ...
        for field in depends_on_fields:
            ...
            if (
                depends_on
                and ("=" in depends_on)
                and re.match("""[\w\.:_]+\s*={1}\s*[\w\.@'"]+""", depends_on)
            ):
                frappe.throw(
                    _("Invalid {0} condition").format(frappe.unscrub(field)),
    ...
    def check_table_multiselect_option(docfield):
        ...
        if not link_field:
            frappe.throw(
                _(
                    "DocType <b>{0}</b> provided for the field <b>{1}</b> must have"
                    " atleast one Link field"
    ...
    def validate_data_field_type(docfield):
        if docfield.fieldtype == "Data" and not (
            ...
            if docfield.options and (docfield.options not in data_field_options):
                ...
                df_options_str = (
                    "<ul><li>"
                    + "</li><li>".join([_(x) for x in data_field_options])
                    + "</ul>"
                )

                frappe.msgprint(
                    text_str + df_options_str,
                    title="Invalid Data Field",
...
def validate_permissions(doctype, for_remove=False, alert=False):
    ...
    def check_level_zero_is_set(d):
        if cint(d.permlevel) > 0 and d.role != "All":
            ...
            if not has_zero_perm:
                frappe.throw(
                    _(
                        "{0}: Permission at level 0 must be set before higher levels"
                        " are set"
                    ).format(get_txt(d))
                )
    ...
    def check_permission_dependency(d):
        ...
        if (d.submit or d.cancel or d.amend) and not d.write:
            frappe.throw(
                _("{0}: Cannot set Submit, Cancel, Amend without Write").format(
                    get_txt(d)
                )
            )
    ...
    def check_if_submittable(d):
        if d.submit and not issubmittable:
            frappe.throw(
                _("{0}: Cannot set Assign Submit if not Submittable").format(get_txt(d))
            )
        elif d.amend and not issubmittable:
            frappe.throw(
                _("{0}: Cannot set Assign Amend if not Submittable").format(get_txt(d))
            )
...
def make_module_and_roles(doc, perm_fieldname="permissions"):
    ...
    try:
        if (
            ...
            frappe.get_doc(
                dict(doctype="Domain", domain=doc.restrict_to_domain)
            ).insert()

        if "tabModule Def" in frappe.db.get_tables() and not frappe.db.exists(
            "Module Def", doc.module
        ):
...
def check_if_fieldname_conflicts_with_methods(doctype, fieldname):
    ...
    method_list = [
        ...
        if isinstance(method, str) and callable(getattr(doc, method))
    ]
...
def check_email_append_to(doc):
    ...
    if doc.subject_field and not subject_field:
        frappe.throw(
            _("Select a valid Subject field for creating documents from Email")
        )
