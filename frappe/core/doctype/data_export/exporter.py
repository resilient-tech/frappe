...
class DataExporter:
    ...
    def add_main_header(self):
        ...
        self.writer.writerow(
            [
                _(
                    'If you are uploading new records, "Naming Series" becomes'
        ...
        self.writer.writerow(
            [
                _(
                    "Only mandatory fields are necessary for new records. You can"
                    " delete non-mandatory columns if you wish."
                )
            ]
        )
        self.writer.writerow(
        ...
        self.writer.writerow(
            [
                _(
                    "You can only upload upto 5000 records in one go. (may be less in"
                    " some cases)"
                )
            ]
        )
        if self.name_field == "parent":
            self.writer.writerow(
                [
                    _(
                        '"Parent" signifies the parent table in which this row must be'
                        " added"
                    )
                ]
            )
            self.writer.writerow(
                [
                    _(
                        'If you are updating, please select "Overwrite" else existing'
                    ...
                    frappe._dict(
                        {
                            ...
                            "reqd": 1,
                            "info": _(
                                "Parent is the name of the document to which the data"
                                " will get added to."
                            ),
                        }
                    ),
            ...
            if self.all_doctypes:
                ...
                for c in self.child_doctypes:
                    for ci, child in enumerate(
                        frappe.db.sql(
                            """select * from `tab{0}`
						where parent=%s and parentfield=%s order by idx""".format(
                                c["doctype"]
                            ),
