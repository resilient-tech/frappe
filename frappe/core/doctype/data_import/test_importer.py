...
class TestImporter(unittest.TestCase):
    ...
    def test_data_import_without_mandatory_values(self):
        ...
        expected_error = (
            "Error: <b>Child 1 of DocType for Import</b> Row #1: Value missing for:"
            " Child Title"
        )
        self.assertEqual(
            frappe.parse_json(import_log[0]["messages"][0])["message"], expected_error
        )
        expected_error = (
            "Error: <b>Child 1 of DocType for Import</b> Row #2: Value missing for:"
            " Child Title"
        )
        self.assertEqual(
            frappe.parse_json(import_log[0]["messages"][1])["message"], expected_error
        )

        self.assertEqual(import_log[1]["row_indexes"], [4])
        self.assertEqual(
            frappe.parse_json(import_log[1]["messages"][0])["message"],
            "Title is required",
        )
    ...
    def test_data_import_update(self):
        ...
        self.assertEqual(
            updated_doc.table_field_1[0].child_description, "child description"
        )
        self.assertEqual(
            updated_doc.table_field_1_again[0].child_title, "child title again"
        )
    ...
    frappe.get_doc(
        {
            ...
            "fields": [
                ...
                {
                    "label": "Child Number",
                    "fieldname": "child_another_number",
                    "fieldtype": "Int",
                },
            ],
        }
    ...
    frappe.get_doc(
        {
            ...
            "fields": [
                ...
                {
                    "label": "Child 2 Date",
                    "fieldname": "child_2_date",
                    "fieldtype": "Date",
                },
                {
                    "label": "Child 2 Number",
                    "fieldname": "child_2_number",
                    "fieldtype": "Int",
                },
                {
                    "label": "Child 2 Number",
                    "fieldname": "child_2_another_number",
                    "fieldtype": "Int",
                },
            ],
        }
