...
class Communication(Document):
        ...
        if self.reference_doctype and self.reference_name:
            ...
            if self.reference_doctype == "Communication":
                ...
                if circular_linking:
                    frappe.throw(
                        _(
                            "Please make sure the Reference Communication Docs are not"
                            " circularly linked."
                        ),
    ...
    def after_insert(self):
        ...
        if (
            self.reference_doctype == "Communication"
            and self.sent_or_received == "Sent"
        ):
            frappe.db.set_value(
                "Communication", self.reference_name, "status", "Replied"
            )
    ...
    def set_delivery_status(self, commit=False):
        ...
        status_counts = Counter(
            frappe.db.sql_list(
                """select status from `tabEmail Queue` where communication=%s""",
                self.name,
            )
        )
    ...
    def set_timeline_links(self):
        ...
        contacts = get_contacts(
            [self.sender, self.recipients, self.cc, self.bcc],
    ...
    def add_link(self, link_doctype, link_name, autosave=False):
        self.append(
            "timeline_links", {"link_doctype": link_doctype, "link_name": link_name}
        )
...
def update_first_response_time(parent, communication):
    if parent.meta.has_field("first_response_time") and not parent.get(
        ...
        if is_system_user(communication.sender):
            ...
            if (
                parent.meta.has_field("first_responded_on")
                and communication.sent_or_received == "Sent"
            ):
                parent.db_set("first_responded_on", first_responded_on)
            parent.db_set(
                "first_response_time",
                round(time_diff_in_seconds(first_responded_on, parent.creation), 2),
            )
