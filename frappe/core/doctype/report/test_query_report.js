...
QUnit.test("Test Query Report", function (assert) {
	...
	frappe.run_serially([
		...
		() =>
			cur_frm.set_value(
				"query",
				"select description,owner,status from `tabToDo`"
			),
		() => cur_frm.save(),
		...
		() => {
			assert.ok(
				$("div.slick-header-column").length == 4,
				"Correct numbers of columns visible"
			);
			//To check if the result is present
			assert.ok(
				$("div.r1:contains(" + random + ")").is(":visible"),
				"Result is visible in report"
			);
			frappe.timeout(3);
		},
		() => done(),
