...
class TestUser(unittest.TestCase):
    def tearDown(self):
        ...
        frappe.db.set_value(
            "System Settings", "System Settings", "enable_password_policy", 0
        )
        frappe.db.set_value(
            "System Settings", "System Settings", "minimum_password_score", ""
        )
        frappe.db.set_value(
            "System Settings", "System Settings", "password_reset_limit", 3
        )
    ...
    def test_delete(self):
        ...
        self.assertTrue(
            not frappe.db.sql(
                """select * from `tabToDo` where owner=%s""", ("_test@example.com",)
            )
        )
    ...
    def test_get_value(self):
        ...
        frappe.db.set_value(
            "Website Settings", "Website Settings", "_test", "_test_val"
        )
        self.assertEqual(
    ...
    def test_delete_user(self):
        new_user = frappe.get_doc(
            dict(
                doctype="User",
                email="test-for-delete@example.com",
                first_name="Tester Delete User",
            )
        ).insert()
        self.assertEqual(new_user.user_type, "Website User")
        ...
        comm = frappe.get_doc(
            {
                ...
                "subject": (
                    "To check user able to delete even if linked with communication"
                ),
                "content": (
                    "To check user able to delete even if linked with communication"
                ),
    ...
    def test_password_strength(self):
        ...
        frappe.db.set_value(
            "System Settings", "System Settings", "enable_password_policy", 1
        )
        frappe.db.set_value(
            "System Settings", "System Settings", "minimum_password_score", 2
        )
    ...
    def test_rate_limiting_for_reset_password(self):
        ...
        frappe.db.set_value(
            "System Settings", "System Settings", "password_reset_limit", 1
        )
        frappe.db.commit()
