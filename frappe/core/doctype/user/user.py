...
class User(Document):
    ...
    def check_demo(self):
        if frappe.session.user == "demo@erpnext.com":
            frappe.throw(
                _(
                    "Cannot change user details in demo. Please signup for a new"
                    " account at https://erpnext.com"
                ),
                title=_("Not Allowed"),
            )
    ...
    def add_system_manager_role(self):
        ...
        if (
            ...
            msgprint(
                _(
                    "Adding System Manager to this User as there must be atleast one"
                    " System Manager"
                )
            )
    ...
    def after_rename(self, old_name, new_name, merge=False):
        ...
        if frappe.db.exists("Chat Profile", old_name):
            frappe.rename_doc(
                "Chat Profile", old_name, new_name, force=True, show_alert=False
            )
    ...
    def find_by_credentials(
        ...
        user = (
            frappe.db.get_value(
                "User", filters=filter, fieldname=["name", "enabled"], as_dict=True
            )
            or {}
        )
        if not user:
...
def remove_user_email_inbox(email_account):
    ...
    for user in users:
        ...
        to_remove = [
            row for row in doc.user_emails if row.email_account == email_account
        ]
    ...
    if user:
        ...
        if (
            ...
            frappe.respond_as_web_page(
                ...
                _(
                    "Too many users signed up recently, so the registration is"
                    " disabled. Please try back in an hour"
                ),
...
def send_token_via_sms(tmp_id, phone_no=None, user=None):
    ...
    if user:
        user_phone = frappe.db.get_value(
            "User", user, ["phone", "mobile_no"], as_dict=1
        )
...
def reset_otp_secret(user):
    ...
    if frappe.session.user in ["Administrator", user]:
        ...
        email_args = {
            "recipients": user_email,
            "sender": None,
            "subject": "OTP Secret Reset - {}".format(otp_issuer or "Frappe Framework"),
            "message": (
                "<p>Your OTP secret on {} has been reset. If you did not perform this"
                " reset and did not request it, please contact your System"
                " Administrator immediately.</p>".format(
        ...
        return frappe.msgprint(
            _(
                "OTP Secret has been reset. Re-registration will be required on next"
                " login."
            )
        )
    else:
