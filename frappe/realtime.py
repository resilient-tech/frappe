...
def get_pending_tasks_for_doc(doctype, docname):
    return frappe.db.sql_list(
        "select name from `tabAsync Task` where status in ('Queued', 'Running') and"
...
def set_task_status(task_id, status, response=None):
    if not response:
        response = {}
    response.update({"status": status, "task_id": task_id})
...
def remove_old_task_logs():
    ...
    files_to_remove = [
        ...
        if is_file_old(_file) and os.path.isfile(_file)
    ]
    for _file in files_to_remove:
        os.remove(_file)
...
def put_log(line_no, line, task_id=None):
    ...
    publish_realtime(
        "task_progress",
        {"message": {"lines": {line_no: line}}, "task_id": task_id},
...
def get_redis_server():
    ...
    if not redis_server:
        ...
        redis_server = Redis.from_url(
            conf.get("redis_socketio") or "redis://localhost:12311"
        )
    return redis_server
