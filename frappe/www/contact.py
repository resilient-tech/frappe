...
def get_context(context):
    ...
    if doc.query_options:
        query_options = [
            opt.strip()
            for opt in doc.query_options.replace(",", "\n").split("\n")
            if opt
        ]
    else:
...
def send_message(subject="Website Query", message="", sender=""):
    ...
    if (
        ...
        frappe.response["message"] = (
            "Sorry: we believe we have received an unreasonably high number of requests"
            " of this kind. Please try later"
        )
        return
