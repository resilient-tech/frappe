...
frappe.views.ListGroupBy = class ListGroupBy {
	...
	get_group_by_count(field) {
		...
		current_filters = current_filters.filter(
			(f_arr) =>
				!f_arr.includes(field === "assigned_to" ? "_assign" : field)
		);
	...
	render_dropdown_items(fields, fieldtype, $dropdown, applied_filter) {
		...
		fields.map((field) => {
			if (field.name === applied_filter) {
				applied_filter_html = this.get_dropdown_html(
					field,
					fieldtype,
					true
				);
		...
		let dropdown_html =
			standard_html + applied_filter_html + dropdown_items_html;
	...
	get_dropdown_html(field, fieldtype, applied = false) {
		...
		let applied_html = applied
			? `<span class="applied"> ${frappe.utils.icon(
					"tick",
					"xs"
			  )} </span>`
			: "";
		return `<li class="group-by-item ${
			applied ? "selected" : ""
		}" data-value="${value}">
