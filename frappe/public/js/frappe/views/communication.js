...
frappe.views.CommunicationComposer = Class.extend({
	...
	toggle_more_options(show_options) {
		show_options =
			show_options || this.dialog.fields_dict.more_options.df.hidden;
	...
	setup_subject_and_recipients: function () {
		...
		if (!this.subject && this.frm) {
			...
			if (!this.subject) {
				...
				if (
					this.frm.meta.subject_field &&
					this.frm.doc[this.frm.meta.subject_field]
				) {
					this.subject = this.frm.doc[this.frm.meta.subject_field];
				} else if (
					this.frm.meta.title_field &&
		...
		this.dialog.on_page_show = function () {
			if (!me.txt) {
				...
				if (last_edited_communication.content) {
					me.dialog.set_value(
						"sender",
						last_edited_communication.sender || ""
					);
					me.dialog.set_value(
						"subject",
						last_edited_communication.subject || ""
					);
					me.dialog.set_value(
						"recipients",
						last_edited_communication.recipients || ""
					);
					me.dialog.set_value(
						"cc",
						last_edited_communication.cc || ""
					);
					me.dialog.set_value(
						"bcc",
						last_edited_communication.bcc || ""
					);
					me.dialog.set_value(
						"content",
						last_edited_communication.content || ""
					);
				}
			}
		};
	...
	setup_print: function () {
		...
		$(fields.attach_document_print.input).click(function () {
			$(fields.select_print_format.wrapper).toggle(
				$(this).prop("checked")
			);
		});
		...
		for (var i = 0, l = this.dialog.fields.length; i < l; i++) {
			...
			if (df.is_cc_checkbox) {
				...
				if (form_values[df.fieldname]) {
					form_values.cc =
						(form_values.cc ? form_values.cc + ", " : "") +
						df.fieldname;
					form_values.bcc =
						(form_values.bcc ? form_values.bcc + ", " : "") +
						df.fieldname;
				}
	...
	save_as_draft: function () {
		if (this.dialog && this.frm) {
			...
			localforage
				.setItem(this.frm.doctype + this.frm.docname, message)
				.catch((e) => {
					if (e) {
						// silently fail
						console.log(e); // eslint-disable-line
						console.warn(
							"[Communication] localStorage is full. Cannot save message as draft"
						); // eslint-disable-line
					}
				});
		}
	...
	delete_saved_draft() {
		if (this.dialog) {
			localforage
				.removeItem(this.frm.doctype + this.frm.docname)
				.catch((e) => {
					if (e) {
						// silently fail
						console.log(e); // eslint-disable-line
		...
		return frappe.call({
			...
			callback: function (r) {
				if (!r.exc) {
					...
					if (
						(frappe.last_edited_communication[me.doc] || {})[me.key]
					) {
	...
	is_print_letterhead_checked: function () {
		if (
			this.frm &&
			$(this.frm.wrapper).find(".form-print-wrapper").is(":visible")
		) {
	...
	setup_earlier_reply: async function () {
		...
		if (this.txt) {
			this.message =
				this.txt + (this.message ? "<br><br>" + this.message : "");
		} else {
			// saved draft in localStorage
			const { doctype, docname } = this.frm || {};
			if (doctype && docname) {
				this.message =
					(await localforage.getItem(doctype + docname)) || "";
			}
		}
		...
		let reply =
			(this.message || "") + (signature ? "<br>" + signature : "");
		let content = "";
		...
		if (this.is_a_reply) {
			...
			let last_email_content =
				last_email.original_comment || last_email.content;
