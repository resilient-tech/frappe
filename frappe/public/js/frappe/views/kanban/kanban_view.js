...
frappe.views.KanbanView = class KanbanView extends frappe.views.ListView {
	static load_last_view() {
		...
		if (route.length === 3) {
			...
			const user_settings =
				frappe.get_user_settings(doctype)["Kanban"] || {};
	...
	get_board() {
		return frappe.db
			...
			.then((board) => {
				this.board = board;
				this.board.filters_array = JSON.parse(
					this.board.filters || "[]"
				);
	...
	on_filter_change() {
		if (
			JSON.stringify(this.board.filters_array) !==
			JSON.stringify(this.filter_area.get())
		) {
	...
	save_kanban_board_filters() {
		...
		frappe
			.call({
				method:
					"frappe.desk.doctype.kanban_board.kanban_board.save_filters",
				args: {
	...
	get_card_meta() {
		...
		if (this.meta.title_field) {
			title_field = frappe.meta.get_field(
				this.doctype,
		...
		// quick entry
		var mandatory = meta.fields.filter(
			(df) => df.reqd && !doc[df.fieldname]
		);
	...
	return get_kanban_boards().then((kanban_boards) => {
		if (kanban_boards) {
			kanban_boards.forEach((board) => {
				let route = `/app/${frappe.router.slug(
					board.reference_doctype
				)}/view/kanban/${board.name}`;
...
frappe.views.KanbanView.show_kanban_dialog = function (doctype, show_existing) {
	...
	function new_kanban_dialog(kanbans, show_existing) {
		...
		let primary_action = () => {
			...
			if (
				!values.selected_kanban ||
				values.selected_kanban == "Create New Board"
			) {
				make_kanban_board(
					values.board_name,
					values.field_name,
				...
				frappe.set_route(
					kanbans.find(
						(kanban) => kanban.name == values.selected_kanban
					).route
				);
			}
		};
	...
	function get_fields_for_dialog(kanban_options, show_existing = false) {
		...
		let fields = [
			...
			},
			{
				...
				mandatory_depends_on:
					'eval: doc.selected_kanban == "Create New Board"',
		...
		const select_fields = frappe.get_meta(doctype).fields.filter((df) => {
			return (
				df.fieldtype === "Select" && df.fieldname !== "kanban_column"
			);
		});
