...
(function () {
		...
		actionCallbacks: {
			...
			update_column: function (updater, col, action) {
				...
				fetch_customization(doctype)
					.then(function (doc) {
						return modify_column_field_in_c11n(
							doc,
							board,
							col.title,
							action
					...
					.call({
						...
						callback: (r) => {
							...
							let updated_cards = [
								{
									name: card.name,
									column: card.to_colname || card.colname,
								},
							];
	...
	frappe.views.KanbanBoard = function (opts) {
		...
		self.update = function (cards) {
			...
			if (
				self.wrapper.find(".kanban").length > 0 &&
				self.cur_list.start !== 0
			) {
		...
		function bind_add_column() {
			var $add_new_column = self.$kanban_board.find(".add-new-column"),
				...
				$compose_column_form = $add_new_column
					.find(".compose-column-form")
					.hide();
			...
			$compose_column_form.keydown(function (e) {
				if (e.which == 13) {
					...
					if (!frappe.request.ajax_count) {
						...
						var title = $compose_column_form.serializeArray()[0]
							.value;
						var col = {
							title: title.trim(),
						};
		...
		function setup_restore_columns() {
			...
			$dropdown
				.find(".dropdown-menu")
				.on("click", "button.restore-column", function () {
			...
			$new_card_area.keydown(function (e) {
				if (e.which == 13) {
					...
					if (!frappe.request.ajax_count) {
						...
						$textarea.val("");
						fluxify
							.doAction("add_card", card_title, column.title)
							.then(() => {
								$btn_add.show();
							});
					}
			...
			get_column_indicators(function (indicators) {
				...
				html += indicators.reduce(function (prev, curr) {
					return (
						prev +
						'<div \
						data-action="indicator" data-indicator="' +
						curr +
						'"\
						class="btn btn-default btn-xs indicator-pill ' +
						curr +
						'"></div>'
					);
				}, "");
	...
	frappe.views.KanbanBoardCard = function (card, wrapper) {
		...
		function make_dom() {
			...
			self.$card = $(
				frappe.render_template("kanban_card", opts)
			).appendTo(wrapper);
		}
		...
		function render_card_meta() {
			...
			if (card.color && frappe.ui.color.validate_hex(card.color)) {
				...
				self.$card
					.find(".kanban-card .kanban-title-area")
					.prepend($div);
			}
		...
		function add_task_link() {
			...
			self.$card.find(".kanban-card-redirect").attr("href", task_link);
		}
		...
		function show_assign_to_dialog(e) {
			...
			self.assign_to = new frappe.ui.form.AssignToDialog({
				...
				callback: function () {
					...
					card.assigned_list = [
						...new Set(card.assigned_list.concat(users)),
					];
	...
	function is_filters_modified(board, cur_list) {
		return new Promise(function (resolve) {
			setTimeout(function () {
				try {
					var list_filters = JSON.stringify(
	...
	function isBound(el, event, fn) {
		...
		handlers.forEach(function (h) {
			if (h.handler.name === fn.name) flag = true;
		});
		return flag;
	}
