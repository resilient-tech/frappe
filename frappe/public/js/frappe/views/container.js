...
frappe.views.Container = Class.extend({
	_intro:
		"Container contains pages inside `#container` and manages \
	...
	has_sidebar: function () {
		...
		flag =
			frappe.ui.pages[route_str] &&
			!frappe.ui.pages[route_str].single_column;
		...
		if (!flag) {
			...
			flag = $(
				`.page-container[data-page-route="${page_route}"] .layout-side-section`
			).length
				? 1
				: 0;
		}
