...
frappe.views.ReportView = class ReportView extends frappe.views.ListView {
	...
	setup_defaults() {
		...
		if (this.report_name) {
			return this.get_report_doc().then((doc) => {
				...
				this.order_by =
					this.report_doc.json.order_by || "modified desc";
	...
	setup_sort_selector() {
		...
		if (this.view_user_settings && this.view_user_settings.group_by) {
			this.group_by_control.apply_settings(
				this.view_user_settings.group_by
			);
		}
	}
	...
	update_row(doc, flash_row) {
		...
		const _flash_row = (rowIndex) => {
			if (!flash_row) return;
			const $row = this.$result.find(
				`.dt-row[data-row-index="${rowIndex}"]`
			);
							...
							fields: [
								...
								},
								{
									label: __("Insert Column Before {0}", [
										datatabe_col.docfield.label.bold(),
									]),
	...
	setup_charts() {
		...
		for (let col of this.columns) {
			...
			if (
				col.docfield &&
				frappe.model.numeric_fieldtypes.includes(col.docfield.fieldtype)
			) {
				y_fields.push({
					label: col.content,
					fieldname: col.id,
		...
		const dialog = new frappe.ui.Dialog({
			...
			fields: [
				...
				},
				{
					...
					default: defaults.y_axes
						? defaults.y_axes.join(", ")
						: null,
				},
				{
					label: __("Chart Type"),
	...
	make_chart() {
		...
		const get_doc = (value, field) =>
			this.data.find((d) => d[field] === value);

		this.$charts_wrapper.removeClass("hidden");

		this.chart = new frappe.Chart(
			this.$charts_wrapper.find(".charts-inner-wrapper")[0],
			{
	...
	get_editing_object(colIndex, rowIndex, value, parent) {
		...
		return {
			...
			setValue: (value) => {
				...
				let fieldname = this.datatable.getColumn(colIndex).docfield
					.fieldname;
				let docname = cell.name;
				let doctype = cell.doctype;

				control.set_value(value);
				return this.set_control_value(
					doctype,
					docname,
					fieldname,
					value
	...
	reorder_fields() {
		...
		let table_fields = this.columns.map((df) => [
			df.field,
			df.docfield.parent,
		]);
	...
	get_fields() {
		...
		const cdt_name_fields = this.get_unique_cdt_in_view().map(
			(cdt) =>
				frappe.model.get_full_column_name("name", cdt) +
				" as " +
				`'${cdt}:name'`
		);
	...
	get_columns_for_picker() {
		...
		if (this.data) {
			has_status_values = frappe.get_indicator(
				this.data[0],
				this.doctype
			);
		}
	...
	setup_columns() {
		...
		for (let f of this.fields) {
			...
			if (column) {
				if (column_widths) {
					column.width =
						column_widths[column.id] || column.width || 120;
				}
	...
	build_column(c) {
		...
		let docfield =
			frappe.meta.docfield_map[doctype || this.doctype][fieldname];
		...
		const editable =
			frappe.model.is_non_std_field(fieldname) && !docfield.read_only;
		...
		// child table column
		const id =
			doctype !== this.doctype ? `${doctype}:${fieldname}` : fieldname;
		...
		return this.columns.map((col) => {
			if (col.docfield.parent !== this.doctype) {
				...
				return {
					...
					editable: Boolean(
						name && this.is_editable(col.docfield, d)
					),
					format: (value) => {
						return frappe.format(
							value,
			...
			if (
				...
				if (status) {
					if (!status[0]) {
						...
						this.add_status_dependency_column(
							dependent_col,
							this.doctype
						);
					}
		...
		const _save_report = (name) => {
			...
			return frappe.call({
				...
				callback: (r) => {
					if (r.exc) {
						frappe.msgprint(
							__("Report was not saved (there were errors)")
						);
						return;
					}
	...
	get_column_widths() {
		if (this.datatable) {
			return this.datatable.datamanager
				.getColumns(true)
				.reduce((acc, curr) => {
	...
	get_columns_totals(data) {
		...
		this.columns.forEach((col, i) => {
			const totals = data.reduce((totals, d) => {
				if (
					col.id in d &&
					frappe.model.is_numeric_field(col.docfield)
				) {
					totals += flt(d[col.id]);
					return totals;
				}
				...
				action: () => {
					const d = new frappe.ui.Dialog({
						...
						primary_action: (values) => {
							...
							this.fields.map((f) =>
								this.add_currency_column(f[0], f[1])
							);
