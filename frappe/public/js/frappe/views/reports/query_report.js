...
frappe.views.QueryReport = class QueryReport extends frappe.views.BaseList {
	...
	add_chart_buttons_to_toolbar(show) {
		if (show) {
			...
			this.create_chart_button = this.page.add_button(
				__("Set Chart"),
				() => {
			...
			if (this.chart_fields || this.chart_options) {
				this.add_to_dashboard_button &&
					this.add_to_dashboard_button.remove();
	...
	add_card_to_dashboard() {
		...
		const dialog = new frappe.ui.Dialog({
			...
			primary_action: (values) => {
				if (!values.label) {
					values.label = `${values.report_function} of ${toTitle(
						values.report_field
					)}`;
				}
	...
	create_dashboard_chart(
		...
		let args = {
			...
			report_name: this.report_name,
			type:
				chart_args.chart_type || frappe.model.unscrub(chart_args.type),
		...
		if (this.chart_fields) {
			...
			chart_name =
				chart_name ||
				`${this.report_name}: ${x_field_title} vs ${y_field_title}`;
				...
				frappe
					...
					.then((settings) => {
						...
						frappe.after_ajax(() => {
							...
							this.report_settings.html_format =
							...
							if (
								this.report_doc.filters &&
								!this.report_settings.filters
							) {
				...
				if (query_params.prepared_report_name) {
					...
					Object.values(this.filters).forEach(function (field) {
						if (filters_from_report[field.fieldname]) {
							field.set_input(
								filters_from_report[field.fieldname]
							);
						}
						if (field.input) {
	...
	get_query_params() {
		const query_string = frappe.utils.get_query_string(
	...
	add_prepared_report_buttons(doc) {
		if (doc) {
			...
			const part2 = __("To get the updated report, click on {0}.", [
				__("Rebuild"),
			]);
		...
		let primary_action = this.primary_action_map[
			this.prepared_report_action
		];

		if (
			!this.primary_button ||
			this.primary_button.text() !== primary_action.label
		) {
	...
	show_warning_or_generate_report() {
		frappe
			...
			.then((reports) => {
				...
				if (reports.length) {
					const message = this.get_queued_prepared_reports_warning_message(
						reports
					);
		...
		if (!missing_mandatory.length) {
			...
			return new Promise((resolve) =>
				...
				let alert_message =
					...
					`</a>`;
				frappe.show_alert(
					{ message: alert_message, indicator: "orange" },
					10
				);
				this.toggle_nothing_to_show(true);
			});
	...
	render_datatable() {
		...
		if (
			...
			if (this.report_settings.get_datatable_options) {
				datatable_options = this.report_settings.get_datatable_options(
					datatable_options
				);
			}
		...
		if (typeof this.report_settings.initial_depth == "number") {
			this.datatable.rowmanager.setTreeDepth(
				this.report_settings.initial_depth
			);
		}
	...
	open_create_chart_dialog() {
		...
		function set_chart_values(values) {
			...
			values.y_fields = values.y_fields
				.map((d) => d.trim())
				.filter(Boolean);
		...
		function preview_chart() {
			...
			if (values.x_field && values.y_fields.length) {
				...
				wrapper
					.find(
						".chart-container .title, .chart-container .sub-title"
					)
					.hide();
				wrapper.show();
		...
		const dialog = new frappe.ui.Dialog({
			...
			fields: [
				...
				},
				{
					...
					default: me.chart_fields
						? me.chart_fields.chart_type
						: "Bar",
				},
				{
					fieldname: "sb_1",
			...
			const format_cell = (value, row, column, data) => {
				if (column.isHeader && !data && this.data) {
					...
					if (column.colIndex === index && !value) {
						...
						in_list(["Currency", "Float"], column.fieldtype)
					) {
	...
	get_filter_values(raise) {
		...
		const mandatory = this.filters.filter(
			(f) => f.df.reqd || f.df.mandatory
		);
	...
	pdf_report(print_settings) {
		...
		const filters_html = this.get_filters_html_for_print();
		const template =
			print_settings.columns || !custom_format
				? "print_grid"
				: custom_format;
	...
	get_filters_html_for_print() {
		...
		return Object.keys(applied_filters)
			.map((fieldname) => {
				const label = frappe.query_report.get_filter(fieldname).df
					.label;
		...
		this.export_dialog = frappe.prompt(
			...
			({ file_format, include_indentation }) => {
				...
				if (file_format === "CSV") {
					...
					const visible_idx = this.datatable.bodyRenderer
						.visibleRowIndices;
		...
		return rows.map((row) => {
			...
			return row.slice(standard_column_count).map((cell, i) => {
				if (cell.column.fieldtype === "Duration") {
					cell.content = frappe.utils.get_formatted_duration(
						cell.content
					);
				}
				if (include_indentation && i === 0) {
					cell.content =
						"   ".repeat(row.meta.indent) + (cell.content || "");
				}
				return cell.content || "";
			});
		});
	...
	get_data_for_print() {
		...
		const rows = this.datatable.datamanager.rowViewOrder
			.map((index) => {
				if (
					this.datatable.bodyRenderer.visibleRowIndices.includes(
						index
					)
				) {
		...
		if (this.raw_data.add_total_row) {
			let totalRow = this.datatable.bodyRenderer
				.getTotalRow()
				.reduce((row, cell) => {
	...
	get_menu_items() {
		let items = [
			...
			},
			{
				label: __("Edit"),
				action: () =>
					frappe.set_route("Form", "Report", this.report_name),
			...
			},
			{
				...
				condition: () =>
					frappe.model.can_export(this.report_doc.ref_doctype),
				standard: true,
			},
			{
					...
					let d = new frappe.ui.Dialog({
						...
						primary_action: (values) => {
							...
							custom_columns.push({
								...
								link_field: this.doctype_field_map[
									values.doctype
								],
							...
							this.custom_columns = this.custom_columns.concat(
								custom_columns
							);
							frappe.call({
								method:
								...
								callback: (r) => {
									...
									const link_field = this.doctype_field_map[
										values.doctype
									];
			...
			},
			{
				...
				condition: () =>
					frappe.model.can_set_user_permissions("Report"),
				standard: true,
			},
		];
						...
						fields: [
							{
								...
								default:
									this.report_doc.is_standard == "No"
										? this.report_name
										: "",
								reqd: true,
							},
		...
		this.data.forEach((row) => {
			dynamic_links.forEach((field) => {
				if (row[field.link_name]) {
					dynamic_doctypes.add(
						row[field.link_name] + ":" + field.fieldname
					);
				}
			});
		});
	...
	setup_report_wrapper() {
		...
		this.$chart = $('<div class="chart-wrapper">')
			.hide()
			.appendTo(this.page.main);
		this.$report = $('<div class="report-wrapper">').appendTo(
			this.page.main
		);
	...
	show_footer_message() {
		...
		this.$report_footer = $(
			`<div class="report-footer text-muted"></div>`
		).appendTo(this.page.main);
		if (this.tree_report) {
		...
		const execution_time_msg = __("Execution Time: {0} sec", [
