...
frappe.defaultDatetimeFormat =
	frappe.defaultDateFormat + " " + frappe.defaultTimeFormat;
...
$.extend(frappe.datetime, {
	convert_to_user_tz: function (date, format) {
		...
		if (frappe.sys_defaults.time_zone) {
			var date_obj = moment
				.tz(date, frappe.sys_defaults.time_zone)
				.local();
		} else {
		...
		return format === false
			? date_obj
			: date_obj.format(frappe.defaultDatetimeFormat);
	},
	...
	is_timezone_same: function () {
		if (frappe.sys_defaults.time_zone) {
			return (
				moment().tz(frappe.sys_defaults.time_zone).utcOffset() ===
				moment().utcOffset()
			);
		} else {
			return true;
	...
	get_user_time_fmt: function () {
		return (
			(frappe.sys_defaults && frappe.sys_defaults.time_format) ||
			"HH:mm:ss"
		);
	},
	...
	get_user_date_fmt: function () {
		return (
			(frappe.sys_defaults && frappe.sys_defaults.date_format) ||
			"yyyy-mm-dd"
		);
	},
	...
	str_to_user: function (val, only_time = false) {
		...
		if (only_time) {
			return moment(val, frappe.defaultTimeFormat).format(user_time_fmt);
		}
