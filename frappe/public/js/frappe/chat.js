...
frappe.datetime.range = class {
	...
	contains(datetime) {
		const contains = datetime.moment.isBetween(
			this.start.moment,
			this.end.moment
		);
		return contains;
...
frappe._.format = (string, object) => {
	for (const key in object) string = string.replace(`{${key}}`, object[key]);
 ...
 */
frappe._.pluralize = (word, count = 0, suffix = "s") =>
	`${word}${count === 1 ? "" : suffix}`;
...
frappe._.copy_array = (array) => {
	if (Array.isArray(array)) return array.slice();
		...
		throw frappe.TypeError(
			`Expected Array, recieved ${typeof array} instead.`
		);
};
...
frappe._.is_empty = (value) => {
	...
	if (value === undefined || value === null) empty = true;
	else if (
		Array.isArray(value) ||
		typeof value === "string" ||
		value instanceof $
	...
	else if (typeof value === "object") empty = Object.keys(value).length === 0;
...
frappe._.squash = (list) =>
	Array.isArray(list) && list.length === 1 ? list[0] : list;
...
frappe.user.first_name = (user) =>
	frappe._.head(frappe.user.full_name(user).split(" "));
...
frappe.Store = class {
	...
	constructor(name) {
		if (typeof name !== "string")
			throw new frappe.TypeError(
				`Expected string for name, got ${typeof name} instead.`
			);
		this.name = name;
	}
...
frappe.Logger = class {
	...
	constructor(name, level) {
		...
		if (!this.level) {
			if (frappe.boot.developer_mode) this.level = frappe.Logger.ERROR;
			else this.level = frappe.Logger.NOTSET;
		}
	...
	debug(message) {
		this.log(message, frappe.Logger.DEBUG);
	}
	info(message) {
		this.log(message, frappe.Logger.INFO);
	}
	warn(message) {
		this.log(message, frappe.Logger.WARN);
	}
	error(message) {
		this.log(message, frappe.Logger.ERROR);
	}
...
frappe.chat.profile.create = (fields, fn) => {
	if (typeof fields === "function") {
		fn = fields;
		fields = null;
	} else if (typeof fields === "string") fields = frappe._.as_array(fields);

	return new Promise((resolve) => {
		frappe.call(
			"frappe.chat.doctype.chat_profile.chat_profile.create",
			{ user: frappe.session.user, exists_ok: true, fields: fields },
			(response) => {
				if (fn) fn(response.message);
...
frappe.chat.profile.update = (user, update, fn) => {
	return new Promise((resolve) => {
		frappe.call(
			"frappe.chat.doctype.chat_profile.chat_profile.update",
...
frappe.chat.room.create = function (kind, owner, users, name, fn) {
	...
	return new Promise((resolve) => {
		frappe.call(
			"frappe.chat.doctype.chat_room.chat_room.create",
			{
				kind: kind,
			...
			(r) => {
				...
				room = {
					...room,
					creation: new frappe.datetime.datetime(room.creation),
				};
...
frappe.chat.room.get = function (names, fields, fn) {
	...
	return new Promise((resolve) => {
		frappe.call(
			"frappe.chat.doctype.chat_room.chat_room.get",
			{ user: frappe.session.user, rooms: names, fields: fields },
			(response) => {
				let rooms = response.message;
				if (rooms) {
					// frappe.api BOGZ! (emtpy arrays are falsified, not good design).
					rooms = frappe._.as_array(rooms);
					rooms = rooms.map((room) => {
						return {
							...room,
							creation: new frappe.datetime.datetime(
								room.creation
							),
...
frappe.chat.room.history = function (name, fn) {
	return new Promise((resolve) => {
		frappe.call(
			"frappe.chat.doctype.chat_room.chat_room.history",
...
frappe.chat.room.search = function (query, rooms) {
	const dataset = rooms.map((r) => {
		...
		else if (r.owner === frappe.session.user)
			return frappe.user.full_name(frappe._.squash(r.users));
		else return frappe.user.full_name(r.owner);
	});
	...
	compare =
		...
		function (a, b) {
			if (a.last_message && b.last_message)
				return frappe.datetime.compare(
					a.last_message.creation,
					b.last_message.creation
				);
			...
			else return frappe.datetime.compare(a.creation, b.creation);
		};
	rooms.sort(compare);
...
frappe.chat.message.typing = function (room, user) {
	frappe.realtime.publish("frappe.chat.message:typing", {
...
frappe.chat.message.update = function (message, update, fn) {
	return new Promise((resolve) => {
		frappe.call(
			"frappe.chat.doctype.chat_message.chat_message.update",
...
frappe.chat.message.sort = (messages) => {
	if (!frappe._.is_empty(messages))
		messages.sort((a, b) =>
			frappe.datetime.compare(b.creation, a.creation)
		);
...
frappe.chat.message.on.update = function (fn) {
	frappe.realtime.on("frappe.chat.message:update", (r) =>
		fn(r.message, r.data)
	);
};
...
frappe.chat.pretty_datetime = function (date) {
	...
	if (today.isSame(instance, "d")) return instance.format("hh:mm A");
	else if (today.isSame(instance, "week")) return instance.format("dddd");
	else return instance.format("DD/MM/YYYY");
};
...
frappe.chat.sound.play = function (name, volume = 0.1) {
	...
	if (frappe._.is_empty($audio)) $(document).append($audio);
...
frappe.chat.emoji = function (fn) {
	return new Promise((resolve) => {
		if (!frappe._.is_empty(frappe.chat.emojis)) {
			if (fn) fn(frappe.chat.emojis);

			resolve(frappe.chat.emojis);
		} else
			$.get(
				"https://cdn.rawgit.com/frappe/emoji/master/emoji",
				(data) => {
...
frappe.chat.website.settings = (fields, fn) => {
	...
	return new Promise((resolve) => {
		frappe
			.call("frappe.chat.website.settings", { fields: fields })
			.then((response) => {
				...
				if (message.enable_from)
					message = {
						...message,
						enable_from: new frappe.datetime.datetime(
							message.enable_from,
							"HH:mm:ss"
						),
					};
				if (message.enable_to)
					message = {
						...message,
						enable_to: new frappe.datetime.datetime(
							message.enable_to,
							"HH:mm:ss"
						),
					};
...
frappe.chat.website.token = (fn) => {
	return new Promise((resolve) => {
		frappe.call("frappe.chat.website.token").then((response) => {
			if (fn) fn(response.message);
...
frappe.components.Button = class extends Component {
	render() {
		...
		return h(
			...
			{
				...props,
				class: `btn ${size && size.class} btn-${props.type} ${
...
frappe.components.FAB = class extends frappe.components.Button {
	render() {
		...
		return h(
			...
			{ ...props, class: `${props.class} ${size && size.class}` },
...
frappe.components.Indicator = class extends Component {
	render() {
		...
		return props.color
			? h("span", { ...props, class: `indicator ${props.color}` })
			: null;
	}
};
...
frappe.components.FontAwesome = class extends Component {
	render() {
		...
		return props.type
			? h("i", {
					...props,
					class: `fa ${props.fixed ? "fa-fw" : ""} fa-${props.type} ${
						props.class
					}`,
			  })
			: null;
...
frappe.components.Octicon = class extends Component {
	render() {
		...
		return props.type
			? h("i", { ...props, class: `octicon octicon-${props.type}` })
			: null;
	}
};
...
frappe.components.Avatar = class extends Component {
	render() {
		...
		const size =
			frappe.components.Avatar.SIZE[props.size] ||
			frappe.components.Avatar.SIZE.medium;

		return h(
			"span",
			{ class: `avatar ${size.class} ${props.class ? props.class : ""}` },
			props.image
...
frappe.Chat.Widget = class extends Component {
	...
	setup(props) {
		...
		this.room.add = (rooms) => {
			...
			frappe.log.info(
				`Subscribing ${frappe.session.user} to Chat Rooms ${names.join(
					", "
				)}.`
			);
			...
			for (const room of rooms)
				if (
					["Group", "Visitor"].includes(room.type) ||
					room.owner === frappe.session.user ||
					room.last_message ||
					room.users.includes(frappe.session.user)
				) {
			...
			const rooms = state.rooms.map((r) => {
				if (r.name === room) {
					...
					if (update.typing) {
						...
						} else update.typing = frappe._.as_array(update.typing);
					}
				...
				if (update.typing) {
					if (!frappe._.is_empty(state.room.typing)) {
						...
						if (!state.room.typing.includes(usr)) {
							update.typing = frappe._.copy_array(
								state.room.typing
							);
							update.typing.push(usr);
						}
			...
			frappe.chat.profile
				...
				.then((profile) => {
					...
					frappe.chat.room.get((rooms) => {
						rooms = frappe._.as_array(rooms);
						frappe.log.info(
							`User ${frappe.session.user} is subscribed to ${
								rooms.length
							} ${frappe._.pluralize("room", rooms.length)}.`
						);

						if (!frappe._.is_empty(rooms)) this.room.add(rooms);
					});
	...
	bind() {
		frappe.chat.profile.on.update((user, update) => {
			frappe.log.warn(
				`TRIGGER: Chat Profile update ${JSON.stringify(
					update
				)} of User ${user}.`
			);
			...
			if ("status" in update) {
				if (user === frappe.session.user) {
					...
					const status = frappe.chat.profile.STATUSES.find(
						(s) => s.name === update.status
					);
					const color = status.color;

					const alert = `<span class="indicator ${color}"/> ${frappe.user.full_name(
						user
					)} is currently <b>${update.status}</b>`;
		...
		frappe.chat.room.on.update((room, update) => {
			frappe.log.warn(
				`TRIGGER: Chat Room ${room} update ${JSON.stringify(
					update
				)} recieved.`
			);
			this.room.update(room, update);
		...
		frappe.chat.room.on.typing((room, user) => {
			if (user !== frappe.session.user) {
				...
				setTimeout(
					() => this.room.update(room, { typing: null }),
					5000
				);
			}
		});
		...
		frappe.chat.message.on.create((r) => {
			...
			// play sound.
			if (state.room.name)
				state.profile.conversation_tones &&
					frappe.chat.sound.play("message");
			else
				state.profile.notification_tones &&
					frappe.chat.sound.play("notification");

			if (
				r.user !== frappe.session.user &&
				state.profile.message_preview &&
				!state.toggle
			) {
				...
				const alert =
					// TODO: ellipses content
					`
				<span data-action="show-message" class="cursor-pointer">
					<span class="indicator yellow"/>
						<span class="avatar avatar-small">
							<span class="avatar-frame" style="background-image: url(${frappe.user.image(
								r.user
							)})"></span>
						</span>
		...
		frappe.chat.message.on.update((message, update) => {
			frappe.log.warn(
				`TRIGGER: Chat Message ${message} update ${JSON.stringify(
					update
				)} recieved.`
			);
		});
	...
	render() {
			...
			for (const email of emails) {
				...
				for (const room of state.rooms) {
					if (room.type === "Direct") {
						if (
							room.owner === email ||
							frappe._.squash(room.users) === email
						)
							exists = true;
					}
				}
		...
		const rooms = state.query
			? frappe.chat.room.search(state.query, state.rooms.concat(contacts))
			: frappe.chat.room.sort(state.rooms);

		const layout = state.span
			? frappe.Chat.Layout.PAGE
			: frappe.Chat.Layout.POPPER;

		const RoomList =
			frappe._.is_empty(rooms) && !state.query
				? h(
						"div",
						{ class: "vcenter" },
						h(
							"div",
							{ class: "text-center text-extra-muted" },
							h("p", "", __("You don't have any messages yet."))
						)
				  )
				: h(frappe.Chat.Widget.RoomList, {
						rooms: rooms,
						click: (room) => {
							if (room.name) this.room.select(room.name);
							else
								frappe.chat.room.create(
									"Direct",
									room.owner,
									frappe._.squash(room.users),
									({ name }) => this.room.select(name)
								);
						},
				  });
		const Room = h(frappe.Chat.Widget.Room, {
			...state.room,
			layout: layout,
			destroy: () => {
				this.set_state({
		...
		const component =
			layout === frappe.Chat.Layout.POPPER
				...
				: h(
						"div",
						{ class: "frappe-chat-popper" },
						h(
							"div",
							{ class: "frappe-chat-popper-collapse" },
							h(
								"div",
								{
									class: "panel panel-default panel-span",
									style: { width: "25%" },
								},
								h("div", { class: "panel-heading" }, ActionBar),
								RoomList
							),
							Room
						)
		...
		return h("div", { class: "frappe-chat" }, component);
	}
};
...
frappe.Chat.Widget.Popper = class extends Component {
	...
	setup(props) {
		...
		if (props.target) $(props.target).click(() => this.toggle());
	...
	toggle(active) {
		let toggle;
		if (arguments.length === 1) toggle = active;
		else toggle = this.state.active ? false : true;
	...
	on_mounted() {
		$(document.body).on(
			"click",
			".page-container, .frappe-chat-close",
			({ currentTarget }) => {
				this.toggle(false);
			}
		);
			...
			? h(
					...
					!props.target
						? h(frappe.components.FAB, {
								...
								icon: state.active
									? "fa fa-fw fa-times"
									: "font-heavy octicon octicon-comment",
...
frappe.Chat.Widget.ActionBar = class extends Component {
	...
	render() {
		...
		return h(
			"div",
			{
				class: `frappe-chat-action-bar ${
					props.class ? props.class : ""
				}`,
			},
			h(
				"form",
				{ oninput: this.change, onsubmit: this.submit },
				h("input", {
					autocomplete: "off",
			...
			!frappe._.is_mobile()
				? h(frappe.Chat.Widget.ActionBar.Action, {
						icon: `octicon octicon-screen-${
							state.span ? "normal" : "full"
						}`,
						onclick: () => {
...
frappe.Chat.Widget.ActionBar.Action = class extends Component {
	render() {
		...
		return h(
			frappe.components.Button,
			{ size: "small", class: "btn-action", ...props },
 ...
 */
frappe.Chat.Widget.RoomList = class extends Component {
	render() {
		const { props } = this;
		...
		return !frappe._.is_empty(rooms)
			? h(
					...
					{
						class:
							"frappe-chat-room-list nav nav-pills nav-stacked",
					},
					rooms.map((room) =>
						h(frappe.Chat.Widget.RoomList.Item, {
							...room,
							click: props.click,
						})
					)
 ...
 */
frappe.Chat.Widget.RoomList.Item = class extends Component {
	render() {
		const { props } = this;
		const item = {};
		...
		if (props.type === "Group") {
			...
			if (!frappe._.is_empty(props.typing)) {
				...
				const names = props.typing.map((user) =>
					frappe.user.first_name(user)
				);
			...
			if (!frappe._.is_empty(props.typing)) item.subtitle = "typing...";
			else if (props.last_message) {
				const message = props.last_message;
				const content = message.content;
		...
		if (props.last_message) {
			item.timestamp = frappe.chat.pretty_datetime(
				props.last_message.creation
			);
 ...
 */
frappe.Chat.Widget.MediaProfile = class extends Component {
	render() {
		const { props } = this;
		const position =
			frappe.Chat.Widget.MediaProfile.POSITION[props.position || "left"];
		const avatar = h(
			"div",
			{ class: `${position.class} media-middle` },
		...
		return h(
			...
			h(
				...
				h(
					"div",
					{
						class: "media-heading ellipsis small",
						style: `max-width: ${
							props.width_title || "100%"
						} display: inline-block`,
					},
					props.title
				),
				props.content
					? h("div", "", h("small", "", props.content))
					: null,
				props.subtitle
					? h(
							"div",
							{ class: "media-subtitle small" },
...
frappe.Chat.Widget.MediaProfile.POSITION = {
	left: { class: "media-left" },
	right: { class: "media-right" },
};

/**
 * @description frappe.Chat.Widget Room Component
 */
frappe.Chat.Widget.Room = class extends Component {
	render() {
		const { props, state } = this;
		const hints = [
			{
				...
				search: function (keyword, callback) {
					if (props.type === "Group") {
						...
						const users = [].concat(
							frappe._.as_array(props.owner),
							props.users
						);
						const grep = users.filter(
							(user) =>
								user !== frappe.session.user &&
								user.indexOf(query) === 0
						);
		...
		return h(
			"div",
			{
				...
				${props.name ? "panel-bg" : ""}
				${
					props.layout === frappe.Chat.Layout.PAGE ||
					frappe._.is_mobile()
						? "panel-span"
						: ""
				}`,
				style: props.layout === frappe.Chat.Layout.PAGE && {
					width: "75%",
					left: "25%",
					"box-shadow": "none",
				},
			...
			props.name &&
				h(frappe.Chat.Widget.Room.Header, {
					...props,
					on_back: props.destroy,
				}),
			props.name
				? !frappe._.is_empty(props.messages)
					...
					: h(
							"div",
							{ class: "panel-body", style: { height: "100%" } },
							h(
								"div",
								{ class: "vcenter" },
								h(
						...
						h(
							...
							h(
								...
								h(
									"p",
									"",
									__("Select a chat to start messaging.")
								)
							)
						)
				  ),
			props.name
				? h(
						"div",
						{ class: "chat-room-footer" },
						h(frappe.chat.component.ChatForm, {
							actions: actions,
							onchange: () => {
...
frappe.Chat.Widget.Room.Header = class extends Component {
	render() {
		const { props } = this;
		...
		if (["Group", "Visitor"].includes(props.type)) {
			...
			if (!frappe._.is_empty(props.typing)) {
				...
				const users = props.typing.map((user) =>
					frappe.user.first_name(user)
				);
				...
				item.subtitle =
					props.type === "Group"
						? `${props.users.length} ${frappe._.pluralize(
								"member",
								props.users.length
						  )}`
						: "";
		...
		const popper =
			props.layout === frappe.Chat.Layout.POPPER || frappe._.is_mobile();

		return h(
			"div",
			{ class: "panel-heading", style: { height: "50px" } }, // sorry. :(
			h(
				"div",
				{ class: "level" },
				popper && frappe.session.user !== "Guest"
					? h(
							frappe.components.Button,
							{ class: "btn-back", onclick: props.on_back },
							h(frappe.components.Octicon, {
								type: "chevron-left",
							})
					  )
				...
				h(
					"div",
					{ class: popper ? "col-xs-2" : "col-xs-3" },
					h(
						"div",
						{ class: "text-right" },
						frappe._.is_mobile() &&
 ...
 */
frappe.chat.component.ChatList = class extends Component {
	on_mounted() {
	...
	render() {
		...
		for (var i = 0; i < this.props.messages.length; ++i) {
			...
			if (
				i === 0 ||
				!frappe.datetime.equal(
					message.creation,
					this.props.messages[i - 1].creation,
					"day"
				)
			)
				messages.push({
					type: "Notification",
					content: message.creation.format("MMMM DD"),
				});
		...
		return h(
			"div",
			{ class: "chat-list list-group" },
			!frappe._.is_empty(messages)
				? messages.map((m) =>
						h(frappe.chat.component.ChatList.Item, { ...m })
				  )
				: null
		);
	}
 ...
 */
frappe.chat.component.ChatList.Item = class extends Component {
	render() {
		const { props } = this;
		...
		return h(
			"div",
			{ class: "chat-list-item list-group-item" },
			props.type === "Notification"
				? h(
						"div",
						{ class: "chat-list-notification" },
						h(
							"div",
							{ class: "chat-list-notification-content" },
							content
						)
				  )
				: h(
...
frappe.chat.component.ChatBubble = class extends Component {
	...
	render() {
		...
		const me = props.user === frappe.session.user;
		const read =
			!frappe._.is_empty(props.seen) &&
			!props.seen.includes(frappe.session.user);
		...
		return h(
			...
			{
				class: `chat-bubble ${
					props.groupable ? "chat-groupable" : ""
				} chat-bubble-${me ? "r" : "l"}`,
				...
				h(
					...
					props.type === "File"
						? h(
								...
								h(frappe.components.FontAwesome, {
									type: "file",
									fixed: true,
								}),
								` ${content.name}`
 ...
 */
frappe.chat.component.ChatForm = class extends Component {
	constructor(props) {
		super(props);
					...
					hint.search(token, (items) => {
						const hints = items
							.map((item) => {
								...
								const replace = token.replace(
									hint.match,
									hint.content ? hint.content(item) : item
								);
								const content = `${sliced.join(
									" "
								)} ${replace}`.trim();
								item = {
									component: hint.component(item),
									content: content,
								};
	...
	render() {
		...
		return h(
			...
			state.hints.length
				? h(
						"ul",
						{ class: "hint-list list-group" },
						state.hints.map((item) => {
							return h(
								"li",
								{ class: "hint-list-item list-group-item" },
								h(
									"a",
									{
			...
			h(
				"form",
				{ oninput: this.onchange, onsubmit: this.onsubmit },
				h(
					"div",
					...
					h("textarea", {
						...
						onkeypress: (e) => {
							if (
								e.which === frappe.ui.keycode.RETURN &&
								!e.shiftKey
							)
								this.onsubmit(e);
						},
					...
					h(
						...
						h(
							...
							h(frappe.components.FontAwesome, {
								class: !frappe._.is_empty(state.content)
									? "text-primary"
									: "text-muted",
								type: "send",
								fixed: true,
...
frappe.chat.component.EmojiPicker = class extends Component {
	render() {
		...
		return h(
			"div",
			{ class: `frappe-chat-emoji dropup ${props.class}` },
			h(
				frappe.components.Button,
				{
					type: "primary",
				...
				const setup_room = (token) => {
					return new Promise((resolve) => {
						frappe.chat.room
							...
							.then((room) => {
								frappe.log.info(
									`Visitor Room Created: ${room.name}`
								);
...
frappe.chat.setup = () => {
	...
	if (frappe.session.user !== "Guest") {
		...
		frappe.chat.profile.create("enable_chat").then(({ enable_chat }) => {
			frappe.log.info(
				`Chat Profile created for User ${frappe.session.user}.`
			);

			if ("desk" in frappe && frappe.sys_defaults) {
				// same as desk?
				const should_render =
					Boolean(parseInt(frappe.sys_defaults.enable_chat)) &&
					enable_chat;
		...
		frappe.chat.profile.on.update((user, profile) => {
			if (user === frappe.session.user && "enable_chat" in profile) {
				frappe.log.warn(
					`Chat Profile update (Enable Chat - ${Boolean(
						profile.enable_chat
					)})`
				);
				const should_render =
		...
		frappe.chat.website
			.settings(["socketio", "enable", "enable_from", "enable_to"])
			.then((settings) => {
				frappe.log.info(
					`Chat Website Setting - ${JSON.stringify(settings)}`
				);
				frappe.log.info(
					`Chat Website Setting - ${
						settings.enable ? "Enable" : "Disable"
					}`
				);
				...
				if (settings.enable_from && settings.enable_to) {
					frappe.log.info(
						`Enabling Chat Schedule - ${settings.enable_from.format()} : ${settings.enable_to.format()}`
					);

					const range = new frappe.datetime.range(
						settings.enable_from,
						settings.enable_to
					);
