      ...
      <div v-if="requests.length != 0" class="result">
        ...
        <div class="result-list">
          <div
            ...
            :key="index"
            @click="route_to_request_detail(request.uuid)"
          >
  ...
  methods: {
    filtered: function (requests) {
      ...
      this.query.pagination.total = Math.ceil(
        requests.length / this.query.pagination.limit
      );
      return requests;
    },
    ...
    refresh: function () {
      frappe
        .call("frappe.recorder.get")
        .then((r) => (this.requests = r.message));
    },
