...
frappe.ui.form.LinkedWith = class LinkedWith {
	...
	load_doctypes() {
		...
		if (this.frm.__linked_doctypes) {
			doctypes_to_load = Object.keys(this.frm.__linked_doctypes).filter(
				(doctype) => !already_loaded.includes(doctype)
			);
		}
	...
	links_not_permitted_or_missing() {
		...
		if (this.frm.__linked_doctypes) {
			links = Object.keys(this.frm.__linked_doctypes).filter(
				frappe.model.can_get_report
			);
		}
		...
		return new Promise((resolve, reject) => (flag ? reject() : resolve()));
	}
