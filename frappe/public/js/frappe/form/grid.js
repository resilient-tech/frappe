...
export default class Grid {
	constructor(opts) {
		...
		if (
			...
			this.template = this.frm.meta.__form_grid_templates[
				this.df.fieldname
			];
		}
	...
	setup_check() {
		this.wrapper.on("click", ".grid-row-check", (e) => {
			...
			if ($check.parents(".grid-heading-row:first").length !== 0) {
				...
				for (
					var ri = (page_index - 1) * page_length;
					ri < result_length;
					ri++
				) {
				...
				var docname = $check
					.parents(".grid-row:first")
					.attr("data-name");
				this.grid_rows_by_docname[docname].select(
					$check.prop("checked")
				);
			}
		...
		selected_children.forEach((doc) => {
			tasks.push(() => {
				if (!this.frm) {
					...
					this.df.data = this.df.data.filter(
						(row) => row.idx != doc.idx
					);
				}
	...
	refresh_remove_rows_button() {
		...
		this.remove_all_rows_button.toggleClass(
			"hidden",
			this.wrapper.find(".grid-heading-row .grid-row-check:checked:first")
				.length
				? false
				: true
		);
	...
	make_sortable($rows) {
		new Sortable($rows.get(0), {
			...
			onUpdate: (event) => {
				let idx =
					$(event.item).closest(".grid-row").attr("data-idx") - 1;
				let doc = this.data[idx % this.grid_pagination.page_length];
	...
	get_modal_data() {
		return this.df.get_data
			? this.df.get_data().filter((data) => {
					if (
						!this.deleted_docs ||
						!in_list(this.deleted_docs, data.name)
					) {
						return data;
					}
			  })
					...
					if (column.df.fieldname == fieldname) {
						if (show) {
							...
							if (row != frappe.ui.form.editable_row) {
								...
								column.field_area &&
									column.field_area.toggle(false);
							}
							//Hide the static area and show field area if it is the editable row
								...
								if (column.field) {
									...
									if (column.field.$input)
										column.field.$input.toggleClass(
											"input-sm",
											true
										);
								}
	...
	setup_allow_bulk_edit() {
		...
		if (
			this.frm &&
			this.frm.get_docfield(this.df.fieldname).allow_bulk_edit
		) {
			// download
			this.setup_download();
	...
	add_custom_button(label, click, position = "bottom") {
		...
		if (!$btn) {
			$btn = $(
				`<button class="btn btn-default btn-xs btn-custom">${__(
					label
				)}</button>`
			)
				.prependTo($wrapper)
