frappe.ui.form.set_user_image = function (frm) {
	...
	var image_actions = frm.sidebar.image_wrapper.find(
	...
	if (image) {
		image =
			window.cordova && image.indexOf("http") === -1
				? frappe.base_url + image
				: image;
		...
		image_actions
			.find(".sidebar-image-change, .sidebar-image-remove")
			.show();
	} else {
...
frappe.ui.form.setup_user_image_event = function (frm) {
	...
	frm.sidebar.image_wrapper.on(
		"click",
		":not(.sidebar-image-actions)",
		(e) => {
			let $target = $(e.currentTarget);
			if ($target.is("a.dropdown-toggle, .dropdown")) {
				return;
			}
			let dropdown = frm.sidebar.image_wrapper.find(
				".sidebar-image-actions .dropdown"
			);
