...
frappe.ui.form.save = function (frm, action, callback, btn) {
	...
	var cancel = function () {
		...
		var workflow_state_fieldname = frappe.workflow.get_state_fieldname(
			frm.doctype
		);
	...
	var check_mandatory = function () {
		...
		$.each(frappe.model.get_all_docs(frm.doc), function (i, doc) {
			...
			$.each(
				frappe.meta.docfield_list[doc.doctype] || [],
				function (i, docfield) {
			...
			if (error_fields.length) {
				...
				if (meta.istable) {
					...
					var message = __("Mandatory fields required in {0}", [
						__(doc.doctype),
					]);
				}
				message =
...
frappe.ui.form.update_calling_link = (newdoc) => {
	...
	var doc = frappe.get_doc(
		frappe._from_link.doctype,
		frappe._from_link.docname
	);
	...
	if (is_valid_doctype()) {
		...
		if (doc && doc.parentfield) {
			...
			$.each(
				frappe._from_link.frm.fields_dict[doc.parentfield].grid
					.grid_rows,
				function (index, field) {
					if (
						field.doc &&
						field.doc.name === frappe._from_link.docname
					) {
