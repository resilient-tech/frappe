...
frappe.ui.form.States = Class.extend({
	init: function (opts) {
		...
		this.update_fields = frappe.workflow.get_update_fields(
			this.frm.doctype
		);
	...
	setup_help: function () {
		...
		this.frm.page.add_action_item(
			...
			function () {
				...
				frappe.workflow
					.get_transitions(me.frm.doc)
					.then((transitions) => {
						const next_actions =
							$.map(
								transitions,
								(d) =>
									`${d.action.bold()} ${__("by Role")} ${
										d.allowed
									}`
	...
	show_actions: function () {
		...
		frappe.workflow.get_transitions(this.frm.doc).then((transitions) => {
			...
			transitions.forEach((d) => {
				if (
					frappe.user_roles.includes(d.allowed) &&
					has_approval_access(d)
				) {
					added = true;
