...
frappe.ui.form.ControlTable = frappe.ui.form.Control.extend({
	make: function () {
		...
		this.$wrapper.on("paste", ":text", function (e) {
			var cur_table_field = $(e.target)
				.closest('div [data-fieldtype="Table"]')
				.data("fieldname");
			...
			if (data.length > 100) {
				...
				frappe.msgprint(
					__(
						"For performance, only the first 100 rows were processed."
					)
				);
			}
			var fieldnames = [];
			var get_field = function (name_or_label) {
				...
				$.each(cur_grid.meta.fields, (ci, field) => {
					...
					if (
						field.fieldname.toLowerCase() === name_or_label ||
						(field.label &&
