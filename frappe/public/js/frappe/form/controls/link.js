...
frappe.ui.form.ControlLink = frappe.ui.form.ControlData.extend({
	...
	setup_awesomeplete: function () {
		...
		this.awesomplete = new Awesomplete(me.input, {
			...
			item: function (item) {
				...
				return $("<li></li>")
					...
					.html(
						`<a><p class="ellipsis" title="${_label}">${html}</p></a>`
					)
					.get(0);
			},
			sort: function () {
			...
			frappe.utils.debounce(function (e) {
				...
				frappe.call({
					...
					callback: function (r) {
						...
						if (args.filters) {
							let filter_string = me.get_filter_description(
								args.filters
							);
							if (filter_string) {
								r.results.push({
		...
		this.$input.on("awesomplete-open", () => {
			this.toggle_container_scroll(
				".modal-dialog",
				"modal-dialog-scrollable"
			);
			this.toggle_container_scroll(
				".grid-form-body .form-area",
				"scrollable"
			);
		...
		this.$input.on("awesomplete-select", function (e) {
			...
			if (me.df.remember_last_selected_value) {
				frappe.boot.user.last_selected_values[me.df.options] =
					item.value;
			}
	...
	toggle_href(doctype) {
		if (
			frappe.model.can_select(doctype) &&
	...
	validate: function (value) {
		...
		return this.validate_link_and_fetch(
			this.df,
			this.get_options(),
			this.docname,
			value
