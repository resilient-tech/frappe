frappe.ui.form.ControlMarkdownEditor = frappe.ui.form.ControlCode.extend({
	...
	make_ace_editor() {
		...
		this.ace_editor_target.wrap(
			`<div class="${this.editor_class}-container">`
		);
		this.markdown_container = this.$input_wrapper.find(
			`.${this.editor_class}-container`
		);
		...
		this.markdown_preview = $(
			`<div class="${this.editor_class}-preview border rounded">`
		).hide();
