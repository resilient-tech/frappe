frappe.ui.form.MultiSelectDialog = class MultiSelectDialog {
	...
	make() {
		...
		this.dialog = new frappe.ui.Dialog({
			title: __("Select {0}", [
				this.doctype == "[Select]" ? __("value") : __(doctype_plural),
			]),
			fields: fields,
			...
			primary_action: function () {
				...
				me.action(
					me.get_checked_values(),
					cur_dialog.get_values(),
					me.args,
					filters_data
				);
			},
					...
					if (Array.isArray(me.setters)) {
						...
						Object.keys(me.setters).forEach(function (setter) {
							frappe.route_options[setter] =
								me.dialog.fields_dict[setter].get_value() ||
								undefined;
						});
					}
	...
	get_checked_values() {
		...
		return this.$results
			...
			.map(function () {
				if (
					$(this).find(".list-row-check:checkbox:checked").length > 0
				) {
					...
					head
						...
						: `<a href="${
								"/app/" +
									...
									"/" +
									result[column] || ""
						  }" class="list-id ellipsis" title="${__(
								result[column] || ""
						  )}">
							${__(result[column] || "")}</a>`
				}
	...
	render_result_list(results, more = 0, empty = true) {
		...
		if (frappe.flags.auto_scroll) {
			this.$results.animate(
				{ scrollTop: me.$results.prop("scrollHeight") },
				500
			);
		}
	}
		...
		if ($.isArray(this.setters)) {
			for (let df of this.setters) {
				filters[df.fieldname] =
					me.dialog.fields_dict[df.fieldname].get_value() ||
					undefined;
			...
			Object.keys(this.setters).forEach(function (setter) {
				...
				if (
					me.dialog.fields_dict[setter].df.fieldtype == "Data" &&
					value
				) {
