...
class Picker {
	...
	setup_picker() {
		...
		`.trim();
		this.color_picker_wrapper = color_picker_template.content.firstElementChild.cloneNode(
			true
		);
		this.parent.appendChild(this.color_picker_wrapper);
		this.color_map = this.color_picker_wrapper.getElementsByClassName(
			"color-map"
		)[0];
		this.color_selector_circle = this.color_map.getElementsByClassName(
			"color-selector"
		)[0];
		this.hue_map = this.color_picker_wrapper.getElementsByClassName(
			"hue-map"
		)[0];
		this.swatches_wrapper = this.color_picker_wrapper.getElementsByClassName(
			"swatches"
		)[0];
		this.hue_selector_circle = this.hue_map.getElementsByClassName(
			"hue-selector"
		)[0];
		this.refresh();
		this.setup_events();
	...
	setup_swatches() {
		...
		this.swatches.forEach((color) => {
			let swatch = swatch_template.content.firstElementChild.cloneNode(
				true
			);
