...
frappe.search.utils = {
			...
			frappe.boot.user.can_create.forEach(function (item) {
				...
				if (level) {
					out.push({
						type: "New",
						label: __("New {0}", [
							me.bolden_match_part(__(item), keywords.substr(4)),
						]),
				...
				if (in_list(frappe.boot.single_types, item)) {
					...
					if (in_list(frappe.boot.user.can_create, item)) {
						...
						out.push({
							type: "New",
							label: __("New {0}", [
								me.bolden_match_part(__(item), keywords),
							]),
							value: __("New {0}", [__(item)]),
							index: level + 0.015,
							match: item,
		...
		frappe.boot.allowed_workspaces.forEach(function (item) {
			...
			if (level > 0) {
				var ret = {
					type: "Workspace",
					label: __("Open {0}", [
						me.bolden_match_part(__(item.name), keywords),
					]),
					...
					if (part.toLowerCase().indexOf(keywords) !== -1) {
						...
						if (result_current_length < result_max_length) {
							...
							if (field_name.length < remaining_length) {
								...
								field_text += me.bolden_match_part(
									field_value,
									keywords
								);
								fields.push(field_text);
	...
	fuzzy_search: function (keywords, _item) {
		...
		if (
			[item, item_without_hyphen].includes(keywords) ||
			[item, item_without_hyphen].some(
				(txt) => txt.toLowerCase().indexOf(keywords) === 0
			)
		) {
