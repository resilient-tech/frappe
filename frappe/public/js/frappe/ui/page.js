...
frappe.ui.Page = Class.extend({
	...
	add_dropdown_item: function ({
		...
		if (icon) {
			$icon = `<span class="menu-item-icon">${frappe.utils.icon(
				icon
			)}</span>`;
		}

		if (shortcut) {
			let shortcut_obj = this.prepare_shortcut_obj(
				shortcut,
				click,
				label
			);
		...
		if (this.is_in_group_button_dropdown(parent, "li > a.grey-link", label))
			return;
	...
	prepare_shortcut_obj(shortcut, click, label) {
		...
		if (frappe.utils.is_mac()) {
			shortcut_obj.shortcut_label = shortcut_obj.shortcut.replace(
				"Ctrl",
				"⌘"
			);
		} else {
	...
	remove_inner_button: function (label, group) {
		...
		if (group) {
			...
			if ($group.length) {
				$group
					.find(
						`.dropdown-item[data-label="${encodeURIComponent(
							label
						)}"]`
					)
					.remove();
	...
	add_view: function (name, html) {
		...
		this.views[name] = element.appendTo(
