...
frappe.ui.GroupBy = class {
	...
	apply_settings(settings) {
		...
		let get_doctype = (name) =>
			name.split(".")[0].replace(/`/g, "").replace("tab", "");
		...
		if (
			...
			!settings.aggregate_on.startsWith("`tab")
		) {
			const aggregate_on_doctype = this.get_aggregate_on_doctype(
				settings
			);
			settings.aggregate_on =
				"`tab" +
	...
	get_aggregate_on_doctype(settings) {
		for (let doctype of Object.keys(this.all_fields)) {
			...
			if (
				dt_fields.find(
					(field) => field.fieldname == settings.aggregate_on
				)
			) {
				return doctype;
			}
