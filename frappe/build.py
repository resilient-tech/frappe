...
def build_missing_files():
    ...
    if missing_assets:
        ...
        command = split(
            "node rollup/build.js --files {0} --no-concat".format(
                ",".join(missing_assets)
            )
        )
...
def get_assets_link(frappe_head):
    ...
    if tag:
        ...
        url = (
            "https://github.com/frappe/frappe/releases/download/{0}/assets.tar.gz"
            .format(tag)
        )
    else:
...
def watch(no_compress):
    ...
    frappe.commands.popen(
        "{pacman} run watch".format(pacman=pacman),
        cwd=frappe_app_path,
        env=get_node_env(),
    )
...
def make_asset_dirs(make_copy=False, restore=False):
    ...
    for app_name in frappe.get_all_apps(True):
        ...
        if app_doc_path:
            symlinks.append(
                [app_doc_path, os.path.join(assets_path, app_name + "_docs")]
            )
        ...
        for source, target in symlinks:
            ...
            if os.path.exists(source):
                if restore:
                    ...
                    if os.path.exists(target):
                        warnings.warn(
                            "Target {target} already exists.".format(target=target)
                        )
                    else:
                        shutil.copytree(source, target)
                else:
