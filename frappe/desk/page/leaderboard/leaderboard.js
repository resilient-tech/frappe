...
class Leaderboard {
		...
		frappe
			...
			.then((config) => {
				...
				for (let doctype in this.leaderboard_config) {
					...
					this.filters[doctype] = this.leaderboard_config[
						doctype
					].fields.map((field) => {
				...
				const _initial_doctype =
					frappe.get_route()[1] || this.doctypes[0];
	...
	render_selected_doctype() {
		this.$sidebar_list.on("click", "li", (e) => {
			...
			this.options.selected_company = frappe.defaults.get_default(
				"company"
			);
			...
			if (
				this.leaderboard_config[this.options.selected_doctype]
					.company_disabled
			) {
	...
	show_leaderboard(doctype) {
		if (this.doctypes.length) {
			if (this.doctypes.includes(doctype)) {
				...
				this.$sidebar_list
					.find(
						`[doctype-value = "${this.options.selected_doctype}"]`
					)
					.trigger("click");
			}
		...
		frappe
			...
			.then((r) => {
				...
				const custom_options = {
					...
					format_tooltip_x: (d) =>
						d[this.options.selected_filter_item],
					height: 140,
				};
	...
	get_leaderboard_data(me, res) {
		if (res && res.message.length) {
			me.message = null;
			me.$container
				.find(".leaderboard-list")
				.html(me.render_list_view(res.message));
	...
	render_list_header() {
		const _selected_filter = this.options.selected_filter.map((i) =>
			frappe.model.unscrub(i)
		);
	...
	get_item_html(item, index) {
		const fields = this.leaderboard_config[this.options.selected_doctype]
			.fields;
		...
		const link = `/app/${frappe.router.slug(
			this.options.selected_doctype
		)}/${item.name}`;
	...
	get_date_range() {
		...
		let date_range_map = {
			...
			"last week": [
				frappe.datetime.add_days(current_date, -7),
				current_date,
			],
			"last month": [
