...
class TranslationTool {
	...
	setup_language_filter() {
		let languages = Object.keys(frappe.boot.lang_dict).map(
			(language_label) => {
		...
		if (translation_language || frappe.boot.lang !== "en") {
			language_selector.set_value(
				translation_language || frappe.boot.lang
			);
		} else {
			frappe.prompt(
				{
	...
	on_translation_click(e, $el) {
		...
		$el.addClass("active");
		this.active_translation = this.messages.find(
			(m) => m.id === message_id
		);
					...
					},
					{
						...
						click: () => {
							let {
								...
								translated_text,
								source_text,
							} = this.form.get_values();
							let existing_value = this.form.translation_dict
								.translated_text;
							if (
	...
	set_status(translation) {
		this.form.get_field("header").$wrapper.find(".translation-status")
			.html(`
	...
	setup_contributions(contributions) {
		...
		if (contributions_exists) {
			...
			this.form
				.get_field("contributed_translations")
	...
	show_confirmation_dialog() {
		this.confirmation_dialog = new frappe.ui.Dialog({
			...
			primary_action: (values) => {
				this.create_translations(values).then(
	...
	show_translation_status_modal(e, $el) {
		...
		frappe
			.xcall("frappe.translate.get_contribution_status", { message_id })
			.then((doc) => {
	...
	update_header() {
		...
		if (edited_translations_count) {
			...
			if (edited_translations_count == 1) {
				message = __("{0} translation pending", [
					edited_translations_count,
				]);
			} else {
				message = __("{0} translations pending", [
					edited_translations_count,
				]);
			}
	...
	get_contribution_indicator_color(message_obj) {
		return message_obj.contribution_status == "Pending"
			? "orange"
			: "green";
	}
