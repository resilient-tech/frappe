...
frappe.setup.SetupWizard = class SetupWizard extends frappe.ui.Slides {
	...
	attach_abort_button() {
		this.$abort_btn = $(
			`<button class='btn btn-secondary btn-xs btn-abort text-muted'>${__(
				"Retry"
			)}</button>`
		);
...
frappe.setup.slides_settings = [
	{
		...
		onload: function (slide) {
			...
			language_field.set_input(
				frappe.setup.data.default_language || "English"
			);
...

	{
		...
		onload: function (slide) {
			if (frappe.session.user !== "Administrator") {
				...
				var $attach_user_image =
					slide.form.fields_dict.attach_user_image.$wrapper;

				if (user_image) {
					$attach_user_image.find(".missing-image").toggle(false);
					$attach_user_image
						.find("img")
						.attr("src", decodeURIComponent(user_image));
		...
		setup_fields: function (slide) {
			...
			if (frappe.setup.data.email) {
				...
				if (frappe.get_gravatar(email, 200)) {
					...
					$attach_user_image
						.find("img")
						.attr("src", frappe.get_gravatar(email, 200));
					$attach_user_image.find(".img-container").toggle(true);
				}
			}
		},
	...
	setup_region_fields: function (slide) {
		...
		*/
		...
		if (frappe.wizard.values.currency) {
			slide
				.get_field("currency")
				.set_input(frappe.wizard.values.currency);
		}

		if (frappe.wizard.values.timezone) {
			slide
				.get_field("timezone")
				.set_input(frappe.wizard.values.timezone);
		}
	},
