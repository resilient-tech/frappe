...
frappe.ui.form.on("Dashboard Chart", {
		...
		if (frm.doc.type == "Heatmap") {
			frappe.db.get_doc("System Settings").then((doc) => {
				...
				frm.set_df_property(
					"heatmap_year",
					"options",
					frappe.dashboard_utils.get_years_since_creation(
						creation_date
					)
				);
			});
				...
				frappe.report_utils
					...
					.then((filters) => {
						if (filters) {
							...
							let filter_values = frappe.report_utils.get_filter_values(
								filters
							);
							frm.set_value(
								"filters_json",
	...
	show_filters: function (frm) {
		...
		frappe.dashboard_utils
			.get_filters_for_chart_type(frm.doc)
			.then((filters) => {
				if (filters) {
					frm.chart_filters = filters;
				}
	...
	render_filters_table: function (frm) {
		...
		let is_document_type =
			frm.doc.chart_type !== "Report" && frm.doc.chart_type !== "Custom";
		...
		table.on("click", () => {
			frm.is_disabled &&
				frappe.throw(__("Cannot edit filters for standard charts"));
