...
class Dashboard(Document):
    def on_update(self):
        ...
        if frappe.conf.developer_mode and self.is_standard:
            export_to_files(
                record_list=[["Dashboard", self.name, self.module + " Dashboard"]],
    ...
    def validate(self):
        ...
        if self.is_standard:
            ...
            if (
                non_standard_docs_map["Dashboard Chart"]
                or non_standard_docs_map["Number Card"]
            ):
...
def get_permission_query_conditions(user):
    ...
    module_condition = (
        "`tabDashboard`.`module` in ({allowed_modules}) or `tabDashboard`.`module` is"
        " NULL".format(allowed_modules=",".join(allowed_modules))
    )
