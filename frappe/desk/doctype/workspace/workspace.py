...
class Workspace(Document):
    def validate(self):
        if (
            ...
            and not disable_saving_as_standard()
        ):
        ...
        if self.is_default and self.name and duplicate_exists:
            frappe.throw(
                _(
                    "You can only have one default page that extends a particular"
                    " standard page."
                )
            )
    ...
    def on_update(self):
        ...
        if frappe.conf.developer_mode and self.is_standard:
            export_to_files(
                record_list=[["Workspace", self.name]], record_module=self.module
            )
