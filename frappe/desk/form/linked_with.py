    ...
    for i, doc in enumerate(docs, 1):
        if validate_linked_doc(doc, ignore_doctypes_on_cancel_all) is True:
            frappe.publish_progress(
                percent=i * 100 / ((len(docs) - len(ignore_doctypes_on_cancel_all))),
    ...
    for dt, link in linkinfo.items():
        ...
        if not linkmeta.get("issingle"):
            fields = [
                ...
                for d in linkmeta.get(
                    "fields",
                    {
                        ...
                        "fieldtype": [
                            "not in",
                            ("Image", "HTML", "Button") + frappe.model.table_fields,
                        ],
                    },
                )
            ...
            fields = [
                ...
                for sf in fields
                if sf and "`tab" not in sf
            ]

            try:
                if link.get("filters"):
                    ret = frappe.get_list(
                        doctype=dt, fields=fields, filters=link.get("filters")
                    )
                    ...
                    if link_fieldnames:
                        ...
                        or_filters = [
                            [dt, fieldname, "=", name] for fieldname in link_fieldnames
                        ]
                        # dynamic link
                        if link.get("doctype_fieldname"):
                            filters.append(
                                [dt, link.get("doctype_fieldname"), "=", doctype]
                            )
                        ret = frappe.get_list(
                            doctype=dt,
                            fields=fields,
...
def get_linked_doctypes(doctype, without_ignore_user_permissions_enabled=False):
    ...
    if without_ignore_user_permissions_enabled:
        return frappe.cache().hget(
            ...
            doctype,
            lambda: _get_linked_doctypes(
                doctype, without_ignore_user_permissions_enabled
            ),
        )
    else:
        return frappe.cache().hget(
            "linked_doctypes", doctype, lambda: _get_linked_doctypes(doctype)
        )
...
def _get_linked_doctypes(doctype, without_ignore_user_permissions_enabled=False):
    ...
    ret.update(
        get_dynamic_linked_fields(doctype, without_ignore_user_permissions_enabled)
    )
...
def get_linked_fields(doctype, without_ignore_user_permissions_enabled=False):
    ...
    links = frappe.get_all(
        "DocField", fields=["parent", "fieldname"], filters=filters, as_list=1
    )
    links += frappe.get_all(
