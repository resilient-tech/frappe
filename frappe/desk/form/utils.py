...
def validate_link():
    ...
    valid_value = frappe.db.get_all(
        options, filters=dict(name=value), as_list=1, limit=1
    )
    ...
    if valid_value:
        ...
        if fetch:
            ...
            fetch_value = None
            try:
                fetch_value = frappe.db.sql(
                    "select %s from `tab%s` where name=%s" % (fetch, options, "%s"),
                    (value,),
                )[0]
            except Exception as e:
                error_message = str(e).split("Unknown column '")
                fieldname = (
                    None if len(error_message) <= 1 else error_message[1].split("'")[0]
                )
                frappe.msgprint(
                    _(
                        "Wrong fieldname <b>{0}</b> in add_fetch configuration of"
            ...
            if fetch_value:
                frappe.response["fetch_values"] = [
                    frappe.utils.parse_val(c) for c in fetch_value
                ]
