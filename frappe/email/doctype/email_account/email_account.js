...
frappe.ui.form.on("Email Account", {
	...
	show_gmail_message_for_less_secure_apps: function (frm) {
		...
		if (frm.doc.service === "GMail") {
			frm.dashboard.set_headline_alert(
				'Gmail will only work if you allow access for less secure \
		...
		frappe.call({
			...
			callback: function (r) {
				if (r.message) {
					...
					frappe.confirm(
						__(
							"Email Domain not configured for this account, Create one?"
						),
						function () {
							frappe.model.with_doctype(
								...
								function () {
									...
									var doc = frappe.model.get_new_doc(
										"Email Domain"
									);
									frappe.set_route(
										"Form",
	...
	set_domain_fields: function (frm, args) {
		if (!args) {
			args = frappe.route_flags.set_domain_values
				? frappe.route_options
				: {};
		}
	...
	email_sync_option: function (frm) {
		...
		if (frm.doc.email_sync_option == "ALL") {
			var msg = __(
				"You are selecting Sync Option as ALL, It will resync all \
	...
	warn_autoreply_on_incoming: function (frm) {
		if (
			...
			var msg = __(
				"Enabling auto reply on an incoming email account will send automated replies \
				to all the synchronized emails. Do you wish to continue?"
			);
			frappe.confirm(msg, null, function () {
				frm.set_value("enable_auto_reply", 0);
				frappe.show_alert({
					message: __("Disabled Auto Reply"),
					indicator: "blue",
				});
			});
		}
