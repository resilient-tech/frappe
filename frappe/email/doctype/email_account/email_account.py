...
class EmailAccount(Document):
    ...
    def validate(self):
        ...
        if duplicate_email_account:
            frappe.throw(
                _(
                    "Email ID must be unique, Email Account already exists for {0}"
        ...
        if self.enable_incoming and self.append_to:
            ...
            if self.append_to not in valid_doctypes:
                frappe.throw(
                    _("Append To can be one of {0}").format(comma_or(valid_doctypes))
                )
    ...
    def check_email_server_connection(self, email_server, in_receive):
        ...
        try:
            ...
            if in_receive and any(map(lambda t: t in message, all_error_codes)):
                ...
                error_message = _(
                    "Authentication failed while receiving emails from Email Account:"
                    " {0}."
                ).format(self.name)
            ...
            if in_receive:
                ...
                description = (
                    frappe.message_log.pop() if frappe.message_log else "Socket Error"
                )
                if test_internet():
    ...
    def get_failed_attempts_count(self):
        return cint(
            frappe.cache().get("{0}:email-account-failed-attempts".format(self.name))
        )
        ...
        if self.enable_incoming:
            ...
            if frappe.local.flags.in_test:
                ...
                try:
                    ...
                    frappe.log_error(
                        title=_("Error while connecting to email account {0}").format(
                            self.name
                        )
                    )
            ...
            for idx, msg in enumerate(incoming_mails):
                ...
                try:
                    args = {
                        ...
                        "seen": None
                        if not seen_status
                        else get_seen(seen_status.get(uid, None)),
                    ...
                    if communication and self.flags.notify:
                        ...
                        communication.notify(
                            attachments=attachments, fetched_from_email_account=True
                        )
            ...
            if len(incoming_mails) > 0 and not frappe.local.flags.in_test:
                frappe.publish_realtime(
                    "new_email",
                    {"account": self.email_account_name, "number": len(incoming_mails)},
                )
    ...
    def insert_communication(self, msg, args=None):
        ...
        if email.message_id:
            ...
            if names:
                ...
                frappe.db.set_value(
                    "Communication", name, "uid", uid, update_modified=False
                )
        ...
        if communication.seen:
            ...
            users = frappe.get_all(
                "User Email", filters={"email_account": self.name}, fields=["parent"]
            )
        ...
        if self.append_to and self.sender_field:
            if self.subject_field:
                if "#" in email.subject:
                    ...
                    if parent_id:
                        parent = frappe.db.get_all(
                            self.append_to, filters=dict(name=parent_id), fields="name"
                        )
                ...
                if not parent:
                    ...
                    parent = frappe.db.get_all(
                        ...
                        filters={
                            ...
                            "creation": (
                                ">",
                                (get_datetime() - relativedelta(days=60)).strftime(
                                    DATE_FORMAT
                                ),
                            ),
                        },
                ...
                if (
                    not parent
                    and len(subject) > 10
                    and is_system_user(email.from_email)
                ):
    ...
    def find_parent_from_in_reply_to(self, communication, email):
        ...
        if in_reply_to:
            if "@{0}".format(frappe.local.site) in in_reply_to:
                    ...
                    if parent_doctype == "Communication":
                        ...
                        if parent.reference_name:
                            ...
                            parent = frappe.get_doc(
                                parent.reference_doctype, parent.reference_name
                            )
            else:
                ...
                if comm:
                    parent = frappe._dict(
                        doctype=comm.reference_doctype, name=comm.reference_name
                    )
    ...
    def build_email_sync_rule(self):
        ...
        if self.email_sync_option == "ALL":
            ...
            last_uid = (
                max_uid + int(self.initial_sync_count or 100) if max_uid == 1 else "*"
            )
    ...
    def check_automatic_linking_email_account(self):
        if self.enable_automatic_linking:
            if not self.enable_incoming:
                frappe.throw(
                    _("Automatic Linking can be activated only if Incoming is enabled.")
                )
            ...
            if frappe.db.exists(
                ...
                frappe.throw(
                    _("Automatic Linking can be activated only for one Email Account.")
                )
    ...
    for email_account in frappe.get_all(
        ...
        if email_account.append_to:
            ...
            for comm in frappe.get_all(
                ...
                filters=[
                    ...
                    {"email_account": email_account.name},
                    {
                        "creation": (
                            ...
                            datetime.now()
                            - timedelta(
                                seconds=(email_account.unreplied_for_mins or 30) * 60
                            ),
                        )
                    },
                    {
                        "creation": (
                            ...
                            datetime.now()
                            - timedelta(
                                seconds=(email_account.unreplied_for_mins or 30)
                                * 60
                                * 3
                            ),
                        )
                ...
                if (
                    frappe.db.get_value(
                        comm.reference_doctype, comm.reference_name, "status"
                    )
                    == "Open"
                ):
