...
class AutoEmailReport(Document):
    ...
    def validate_report_count(self):
        ...
        count = frappe.db.sql(
            "select count(*) from `tabAuto Email Report` where user=%s and enabled=1",
            self.user,
        )[0][0]
        if count > max_reports_per_user + (-1 if self.flags.in_insert else 0):
            frappe.throw(
                _("Only {0} emailed reports are allowed per user").format(
        ...
        if self.format not in valid_report_formats:
            frappe.throw(
                _(
                    ...
                    frappe.bold(self.format),
                    frappe.bold(", ".join(valid_report_formats)),
                )
            )
...
def send_daily():
    ...
    for report in enabled_reports:
        ...
        try:
            ...
            frappe.log_error(
                e,
                _("Failed to send {0} Auto Email Report").format(
