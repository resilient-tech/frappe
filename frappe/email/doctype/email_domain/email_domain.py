...
class EmailDomain(Document):
            ...
            try:
                if self.get("use_ssl_for_outgoing"):
                    ...
                    sess = smtplib.SMTP_SSL(
                        (self.smtp_server or "").encode("utf-8"),
                    ...
                    if self.use_tls and not self.smtp_port:
                        self.smtp_port = 587
                    logger.info(
                        "Checking outgoing SMTP email server {host}:{port}"
                        " STARTTLS={tls}...".format(
                    ...
                    sess = smtplib.SMTP(
                        cstr(self.smtp_server or ""), cint(self.smtp_port) or None
                    )
                sess.quit()
            except Exception as e:
                logger.warn(
    ...
    def on_update(self):
        ...
        for email_account in frappe.get_all(
            "Email Account", filters={"domain": self.name}
        ):
            try:
