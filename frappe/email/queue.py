...
def send(
    ...
    email_account = get_outgoing_email_account(
        True, append_to=reference_doctype, sender=sender
    )
    ...
    if should_append_unsubscribe:
        unsubscribe_link = get_unsubscribe_message(
            unsubscribe_message, expose_recipients
        )
    ...
    for email in get_queue():
        ...
        if email.name:
            ...
            if from_test:
                ...
                enqueue(
                    method="frappe.email.queue.send_one", queue="short", **send_one_args
                )
...
def send_one(email, smtpserver=None, auto_commit=True, now=False):
    ...
    if email.communication:
        frappe.get_doc("Communication", email.communication).set_delivery_status(
            commit=auto_commit
        )
...
def prepare_message(email, recipient, recipients_list):
    ...
    email_account = get_outgoing_email_account(
        raise_exception_not_set=False, sender=email.sender
    )
    if frappe.conf.use_ssl and email_account.track_email_status:
        ...
        message = message.replace(
            "<!--email open check-->", quopri.encodestring("".encode()).decode()
        )
    ...
    if email.expose_recipients == "header":
        ...
        if email.expose_recipients == "footer":
            ...
            if email_sent_cc:
                ...
                ).format(email_sent_to, email_sent_cc)
            else:
    ...
    if email.attachments:
        ...
        for attachment in attachments:
            ...
            if fid:
                ...
                attachment.update(
                    {"fname": _file.file_name, "fcontent": fcontent, "parent": message}
                )
