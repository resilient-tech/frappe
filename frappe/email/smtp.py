...
def get_outgoing_email_account(
    raise_exception_not_set=True, append_to=None, sender=None
):
    ...
    if not (
        ...
        if sender_email_id:
            ...
            email_account = _get_email_account(
                {"enable_outgoing": 1, "email_id": sender_email_id}
            )
        ...
        if (
            ...
            frappe.throw(
                _(
                    "Please setup default Email Account from Setup > Email > Email"
                    " Account"
                ),
        ...
        frappe.local.outgoing_email_account[
            append_to or sender_email_id or "default"
        ] = email_account
        ...
        email_account.update(
            {
                ...
                "use_tls": cint(frappe.conf.get("use_tls") or 0)
...
class SMTPServer:
    ...
    def setup_email_account(self, append_to=None, sender=None):
        ...
        if self.email_account:
            ...
            self.login = (
                getattr(self.email_account, "login_id", None)
            ...
            if not self.email_account.no_smtp_authentication:
                if self.email_account.ascii_encode_password:
                    self.password = frappe.safe_encode(
            ...
            self.append_emails_to_sent_folder = (
                self.email_account.append_emails_to_sent_folder
            )
    ...
    def sess(self):
        ...
        if not getattr(self, "server"):
            err_msg = _(
                "Email Account not setup. Please create a new Email Account from Setup"
                " > Email > Email Account"
            )
        ...
        try:
            if self.use_ssl:
                ...
                self._sess = smtplib.SMTP(
                    cstr(self.server or ""), cint(self.port) or None
                )
