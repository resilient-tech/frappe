...
frappe.ui.form.PrintView = class {
	...
	setup_toolbar() {
		...
		this.page.add_button(frappe.utils.icon("refresh"), () =>
	...
	preview() {
		...
		this.get_print_html((out) => {
			...
			const print_height_inches = frappe.dom.pixel_to_inches(
				print_height
			);
	...
	setup_print_format_dom(out, $print_format) {
		...
		setTimeout(() => {
			$print_format.height(
				this.$print_format_body.find(".print-format").outerHeight()
			);
		}, 500);
	}
	...
	hide() {
		if (
			this.frm.setup_done &&
			this.frm.page.current_view_name === "print"
		) {
		...
		let w = window.open(
			frappe.urllib.get_full_url(
				method +
					...
					"&settings=" +
					encodeURIComponent(
						JSON.stringify(this.additional_settings)
					) +
	...
	get_mapped_printer() {
		...
		if (print_format_printer_map[this.frm.doctype]) {
			return print_format_printer_map[this.frm.doctype].filter(
				(printer_map) =>
					printer_map.print_format == this.selected_format()
			);
		} else {
			return [];
		}
	...
	printer_setting_dialog() {
		...
		frappe.ui.form.qz_get_printer_list().then((data) => {
			...
			const dialog = new frappe.ui.Dialog({
				...
				primary_action: () => {
					let printer_mapping = dialog.get_values()[
						"printer_mapping"
					];
					if (printer_mapping && printer_mapping.length) {
						...
						let has_duplicate = print_format_list.some(
							(item, idx) =>
								print_format_list.indexOf(item) != idx
						);
						if (has_duplicate)
							frappe.throw(
								__(
