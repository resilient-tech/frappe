...
frappe.pages["print-format-builder"].on_page_show = function (wrapper) {
	...
	if (route.length > 1) {
		frappe.model.with_doc("Print Format", route[1], function () {
			frappe.print_format_builder.print_format = frappe.get_doc(
				"Print Format",
				route[1]
			);
		...
		if (frappe.route_options.make_new) {
			...
			frappe.print_format_builder.setup_new_print_format(
				doctype,
				name,
				based_on
			);
...
frappe.PrintFormatBuilder = Class.extend({
	...
	make: function () {
		...
		this.page.sidebar = $(
			'<div class="print-format-builder-sidebar"></div>'
	...
	show_start: function () {
		this.page.main.html(
			frappe.render_template("print_format_builder_start", {})
		);
	...
	setup_new_print_format: function (doctype, name, based_on) {
		frappe.call({
			method:
				"frappe.printing.page.print_format_builder.print_format_builder.create_custom_format",
			args: {
				doctype: doctype,
				name: name,
				based_on: based_on,
	...
	init_visible_columns: function (f) {
		...
		$.each(frappe.get_meta(f.options).fields, function (i, _f) {
			if (
				!in_list(["Section Break", "Column Break"], _f.fieldtype) &&
				!_f.print_hide &&
				f.label
			) {
	...
	setup_sortable: function () {
		...
		Sortable.create(
			this.page.sidebar
				.find(".print-format-builder-sidebar-fields")
				.get(0),
			{
				group: {
					name: "field",
		...
		// section sorting
		Sortable.create(
			this.page.main.find(".print-format-builder-layout").get(0),
	...
	setup_field_filter: function () {
		...
		this.page.sidebar.find(".filter-fields").on("keyup", function () {
			...
			me.page.sidebar.find(".field-label").each(function () {
				var show =
					!text ||
					$(this).text().toLowerCase().indexOf(text.toLowerCase()) !==
						-1;
	...
	setup_html_data: function () {
		...
		this.page.main.find('[data-fieldtype="Custom HTML"]').each(function () {
			...
			var html =
				me.custom_html_dict[
					parseInt(content.attr("data-custom-html-id"))
				].options;
			content.data("content", html);
		});
	},
	update_columns_in_section: function (
		section,
		no_of_columns,
		new_no_of_columns
	) {
		...
		if (new_no_of_columns < no_of_columns) {
			...
			for (var i = no_of_columns; i > new_no_of_columns; i--) {
				var $col = $(
					section.find(".print-format-builder-column").get(i - 1)
				);
				var prev = section
			...
			for (var i = no_of_columns; i < new_no_of_columns; i++) {
				...
				me.setup_sortable_for_column(
					col.find(".print-format-builder-column").get(0)
				);
			}
			// resize
			resize();
	...
	setup_add_section: function () {
		...
		this.page.main
			...
			.on("click", function () {
				...
				me.setup_sortable_for_column(
					$section.find(".print-format-builder-column").get(0)
				);
			});
	},
	setup_edit_heading: function () {
		var me = this;
		var $heading = this.page.main.find(
			".print-format-builder-print-heading"
		);
		...
		this.page.main.on("click", ".select-columns", function () {
			...
			$.each(doc_fields, function (j, f) {
				if (
					...
					!in_list(["Section Break", "Column Break"], f.fieldtype) &&
					f.label
				) {
					fields.push(f);
				}
			...
			var get_width_input = function (fieldname) {
				return $body.find(
					".column-width[data-fieldname='" + fieldname + "']"
				);
			};
	...
	setup_edit_custom_html: function () {
		...
		this.page.main.on("click", ".edit-html", function () {
			me.get_edit_html_dialog(
				...
				__("Custom HTML"),
				$(this)
					.parents(".print-format-builder-field:first")
					.find(".html-content")
			);
		});
	},
