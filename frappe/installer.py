...
def install_app(name, verbose=False, set_as_patched=True):
    ...
    sync_for(
        name, force=True, sync_everything=True, verbose=verbose, reset_permissions=True
    )
...
def remove_app(app_name, dry_run=False, yes=False, no_backup=False, force=False):
    ...
    for module_name in modules:
        ...
        doctypes_with_linked_modules = [
            x
            for x in all_doctypes_with_linked_modules
            if frappe.db.exists("DocType", x)
        ]
        for doctype in doctypes_with_linked_modules:
            for record in frappe.get_all(
                doctype, filters={"module": module_name}, pluck="name"
            ):
...
def init_singles():
    singles = [
        single["name"]
        for single in frappe.get_all("DocType", filters={"issingle": True})
    ]
        ...
        for line in f:
            if head in line:
                ...
                apps_list = (
                    app_rows
                    if all(
                        isinstance(app_row, (tuple, list, set)) for app_row in app_rows
                    )
                    else (app_rows,)
                )
