context("Depends On", () => {
	before(() => {
		...
		return cy
			...
			.then((frappe) => {
				return frappe.xcall(
					"frappe.tests.ui_test_helpers.create_child_doctype",
					{
						...
						fields: [
							...
							},
							{
								...
								mandatory_depends_on:
									"eval:doc.test_field=='Some Value'",
								read_only_depends_on:
									"eval:doc.test_field=='Some Other Value'",
							},
							{
	...
	it("should set the field as read only depending on other fields value", () => {
		...
		cy.get("body").click();
		cy.get('.control-input [data-fieldname="dependant_field"]').should(
			"be.disabled"
		);
		cy.fill_field("test_field", "Random Value");
		cy.get("body").click();
